...
;; [PM] 4.0.1 bumped version to 4.1 for SP 4.0.1
(defconst sicstus-support-version '(4 . 1)
  "Version of sicstus-support library as a pair (MAJOR . MINOR)")

;; [PM] 3.11.1 configure fills in version info here
(defvar sicstus-version '(4 . 0)
...
(defun sicstus-prolog-menu ()
  ...
  (list
   ...
   (list "Debugging"
         ...
         ["Zip" prolog-zip-on :active (prolog-atleast-version '(3 . 7))]
   ...
   '("Source level debugging"
     ["Active" prolog-enable-sicstus-sd :active (prolog-atleast-version '(3 . 7))]
     ["Disable" prolog-disable-sicstus-sd :active (prolog-atleast-version '(3 . 7))]
     )
...
;; ;; proc and global variable pltrace-process is always the same
;; (defun pltrace-execute-command (proc command)
;;   (let* ((pos (pltrace-find command "\n"))
;;          (first (substring command 0 pos))
;;          (second (substring command pos))
;;          (param-list (pltrace-split-command first)))
;;     (if (= 0 (length second))
;;         (pltrace-execute-normal-command (car param-list) (cdr param-list))
;;       (pltrace-execute-extended-command (car param-list) (cdr param-list)
;;                                       (substring second 1)))))
;;


;; executes commands f, b, u, k
  ...
  (cond ((string= "u" command)
	...
	(t
	 (let ((port (car param-list))
	   ...
	   (cond ((string= "f" command)
		  (pltrace-file-show port line-num rest))
		 ((string= "b" command)
		  (pltrace-buffer-show port line-num rest)))))))
...
;; with the overlay-arrow corresponding to port
(defun pltrace-file-show (port line-num file-name)
;  (pltrace-show port line-num (find-file-noselect file-name)))

;  (message "pltrace, line %d" line-num)
  ...
  (let ((bufs (buffer-list))
    ...
    (pltrace-show port line-num buffer)))

;; displays buffer named buffer-name and highlights line line-num
;; with the overlay-arrow corresponding to port
(defun pltrace-buffer-show (port line-num buffer-name)
  (pltrace-show port line-num (get-buffer buffer-name)))
...
(defun pltrace-show (port line-num buffer)
  (let ((pltrace-window (display-buffer buffer)))
  (save-excursion
    (set-buffer buffer)
...
(defun pltrace-current ()  
  "Turn on source level debugging for the current buffer."
   (interactive)
...
;; Prolog-trace
(defun pltrace-on ()  
  "Turn on source level debugging for the *prolog* buffer."
   (interactive)
