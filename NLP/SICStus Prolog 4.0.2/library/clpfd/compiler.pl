...
fd_expansion(Head1, Body, Type,
	     [(:-clpfd:'$fd_install'(Spec, Module, Type, InfoLength, Info))]) :-
...
compile(table(Table), Type, Codes, M) :- !,
	compile_table(Table, Body),
	compile(Body, Type, Codes, M).
...
must_be_fd_integer(Arg, Constraint, ArgNo) :-
	integer(Arg), !,
	illarg(representation('CLPFD integer overflow'), Constraint, ArgNo, Arg).
must_be_fd_integer(Arg, Constraint, ArgNo) :-
	illarg(type(integer), Constraint, ArgNo, Arg).
...
%%% indexical expansion of table(Table)

compile_table(Table1, Body) :-
	Table1 = [Row|_],
	length(Row, N),
	length(Args, N),
	numbervars(Args, 0, _),
	table_fdsets(Table1, Table2, []),
	table_indexicals(0, N, Args, Table2, Ixs),
	commafy(Ixs, Body).

table_indexicals(N, N, _, _, []) :- !.
table_indexicals(I, N, [X|Args], Rows1, [(X in Expr)|Exps]) :-
	J is I+1,
	append(Args, [X], Args2),
	rotate_rows(Rows1, Rows2, J),
	table_expr(N, Args2, Rows2, Expr),
	table_indexicals(J, N, Args2, Rows2, Exps).

table_expr(N, Args, Rows1, Expr) :-
	all_events(0, N, Rows1, Events),
	all_expand(Rows1, Events, Rows2, []),
	sort(Rows2, Rows3),
	table_tree(Rows3, Tree, N),
	tree_expr(Tree, Args, Expr).

rotate_rows([], [], _).
rotate_rows([[X|Row1]|Rows1], [Row2|Rows2], J) :-
	append(Row1, [X], Row2),
	rotate_rows(Rows1, Rows2, J).

tree_expr(KL1, [_], Expr) :- !,
	keys_and_values(KL1, Set, _),
	fdset_to_range(Set, Expr).
tree_expr(KL1, [X|Args], unionof(B,dom(X),switch(B,KL2))) :-
	treelist_map(KL1, Args, KL2, []).

treelist_map([], _) --> [].
treelist_map([[Min|Max]-Tree|KL1], Args) -->
	{tree_expr(Tree, Args, Expr)},
	switch_items(Min, Max, Expr),
	treelist_map(KL1, Args).

switch_items(Min, Max, _) --> {Min>Max}, !.
switch_items(Min, Max, Expr) --> [Min-Expr],
	{Min1 is Min+1},
	switch_items(Min1, Max, Expr).
...
fd_goal_expansion(X in_set Set, _, Goal) :-
	in_set_goal(X, Set, Goal).
fd_goal_expansion(X #= Y, _, clpfd:Goal) :-
...
fd_goal_expand_in(X, Expr, Goal) :-
	(   on_exception(_, set_expression(Expr, Set), fail)
	->  in_set_goal_1(X, Set, Goal)
