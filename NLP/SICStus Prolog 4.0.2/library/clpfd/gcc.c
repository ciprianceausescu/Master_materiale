...
#define gcc_alloc(A1,A2,A3,A4,A5) gcc_alloc(HIDDEN_ARG, A1,A2,A3,A4,A5)
#define gcc_refresh(A1,A2,A3) gcc_refresh(HIDDEN_ARG, A1,A2,A3)
#define gcc_filter(A1,A2) gcc_filter(HIDDEN_ARG, A1,A2)
#define costgcc_filter(A1,A2,A3,A4,A5) costgcc_filter(HIDDEN_ARG, A1,A2,A3,A4,A5)
#define costgcc_in_kilter(A1,A2,A3) costgcc_in_kilter(HIDDEN_ARG, A1,A2,A3)
#define costgcc_prune(A1,A2,A3) costgcc_prune(HIDDEN_ARG, A1,A2,A3)
#define compress_edges(A1) compress_edges(HIDDEN_ARG, A1)
#define gcc_new_domain(A1,A2,A3,A4,A5) gcc_new_domain(HIDDEN_ARG, A1,A2,A3,A4,A5)
...
struct gcc_graph
{
  ...
  struct {                      /* source, vals, vars, target */
    ...
    long *dual[2];		/* for cost-based pruning */
  } vertex;

  /* cost fields, persistent */
  long costmin[2];		/* min (-max) element of cost matrix */
  long *cost;			/* cost matrix, parameter */
  long *costbase;		/* cost minus a fixed offset */
  long *regret[2];		/* regret matrix, parameter */
  long *regretbase[2];		/* regret minus a fixed offset */
...
*/
static int
res(struct gcc_graph *pdata, VERTEX u, VERTEX v, int b)
{
#if EXPLICIT_FLOW
  if (u == 0)                   /* from start to target or to vals */
    return v > pdata->nvals ? FLOW(v,u,b) : CAPACITY(v) - FLOW(u,v,b);
  if (v == 0)                   /* to start from target or from vals */
    return u > pdata->nvals ? pdata->nvars - FLOW(u,v,b) : FLOW(v,u,b) - LIMIT(u);
  /* other forward or backward edges */
  return u < v ? 1 - FLOW(u,v,b) : FLOW(v,u,b);
#else
  int n = pdata->nnodes;
  
  if (u == 0)                   /* from start to target or to vals */
    return v > pdata->nvals ? MATE(0,b) : CAPACITY(v) - MATE(v,b);
  if (v == 0)                   /* to start from target or from vals */
    return u > pdata->nvals ? pdata->nvars - MATE(0,b) : MATE(u,b) - LIMIT(u);
  /* other forward or backward edges */
  if (u == n-1)
    return v==0 ?  -MATE(0,b) : MATE(v,b) != -1;
  if (v == n-1)
    return u==0 ? 1+MATE(0,b) : MATE(u,b) == -1;
  return u < v ? MATE(v,b)!=u : MATE(u,b)==v;
#endif
}
...
*/
static void 
bfs(struct gcc_graph *pdata, int b)
{
  int top = 0, bottom = 0;
...
*/
static void 
blocking_flow(struct gcc_graph *pdata, int b)
{
		  ...
		  VERTEX x)
{
  ...
  for (i = 0; i < degree; ++i) {
    ...
    if (0 < res(pdata, x, v, 0)) {
      int m = VISITED(v) ? VISITED(v) : sccs_visit(v);
      if (m < min)
	min = m;
    }
  }
...
gcc_alloc MAGIC (HIDDEN_PROTO Argdecl,
		 int novars,
		 int novals,
		 int flags,
		 TAGGED handle)
{
...
#endif
    (!flags ? 0 : 3*novars*novals*sizeof(long)) +
    (!flags ? 0 : 2*nodes*sizeof(long)) +
  ...
  if (!flags) {
    ...
    ptr += novars*novals*sizeof(long);
    for (b=0; b<2; b++) {
      pdata->regret[b] = (long *)ptr;
      pdata->regretbase[b] = pdata->regret[b] - (novals*novals + novals + 1);
      ptr += novars*novals*sizeof(long);
      pdata->vertex.dual[b] = (long *)ptr;
      ptr += nodes*sizeof(long);
    }
...
/* At present, called only from one place. Could be INLINEd. */
static void 
gcc_init(struct gcc_graph *pdata,
	 TAGGED vals,
	 TAGGED tmat)
{
...
*/
static void 
gcc_refresh MAGIC (HIDDEN_PROTO
		   struct gcc_graph *pdata,
		   int flags,
		   SP_BOOL incremental)
{
...
static void
maintain_capacities(struct gcc_graph *pdata, int b)
{
  int i, j;
...
static SP_BOOL
adjust_count_bounds(struct gcc_graph *pdata, int flags, SP_BOOL *change)
{
  int rc = 0;
  int minsum = 0;
  int maxsum = 0;
  int diff;
  int i, j, lb, ub, jlim, f, b;
...
/* precond: n>=1 */
static TAGGED 
gcc_new_domain MAGIC (HIDDEN_PROTO
		      struct gcc_graph *pdata,
		      Argdecl,
		      int i,
		      int n,
...
{
  int nvals = pdata->nvals;
  long differential = DUAL(v,b) - DUAL(u,b);
  long _cost;
...
/* reset distances */
static void
costgcc_reset_dist_row(struct gcc_graph *pdata, VERTEX u, int b)
{
  ...
  if (u==0) {
    ...
    int k=0;
    long redcost = reduced_cost(pdata,u,0,b);
    ...
    for (; j<top; j++) { /* [VAL,VAR] entries */
      ...
      VERTEX v = ACTIVE(j);
      long redcost = reduced_cost(pdata,u,v,b);
      int k0 = k;
...
static SP_BOOL
costgcc_mark(struct gcc_graph *pdata, int b)
{
  int nvaltargets = pdata->nvaltargets;  
  int nvals = pdata->nvals;
  long _cost, max_regret, min_regret;
  int i, j;
  ...
  for (i=0; i<nvaltargets; i++) {
	...
	if (FLOW(val1,var1,b)) {
	  ...
	  if (deg2>1) { /* otherwise there will be no val2 */
	    long relcost1 = COST(val1,var1,b) - DUAL(val1,b);
	    for (j2=1; j2<=deg2; j2++) {
	      VERTEX /*value a*/val2 = NEIGH(var1,j2);

	      if (val2!=val1) {
		long dist = pdata->distrow[val2];

		if (dist==CLPFD_MAXINT) { /* separate case to avoid oflo */
		  max_regret = dist;
		  REGRET(val2,var1,b) = dist;
		} else {
		  long relcost2 = COST(val2,var1,b) - DUAL(val2,b);
		  long regret = dist-relcost1+relcost2;
...
static SP_BOOL
costgcc_prune MAGIC (HIDDEN_PROTO
		     struct gcc_graph *pdata,
		     Argdecl,
		     SP_BOOL *change)
{
  ...
  if (pdata->max_regret[0] > pdata->threshold[0] ||
      ...
      cost_has_hole) {
    long *threshold = pdata->threshold;
    long regret[2];
 ...
 */
static SP_BOOL
costgcc_in_kilter MAGIC (HIDDEN_PROTO
			 struct gcc_graph *pdata,
			 long accum,
			 int b)
{
...
static SP_BOOL 
costgcc_filter MAGIC (HIDDEN_PROTO
		      struct gcc_graph *pdata,
		      long accum,
		      SP_BOOL incremental,
		      int b,
...
/*    X(0) = RefTerm(State0); */
  ...
  flags = GetSmall(tlvec);
  accum = GetSmall(CTagToArg(X(0),7));
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    fd.gdata = pdata = gcc_alloc(w, novars, novals, flags, handle);
  ...
  gcc_refresh(pdata,flags,incremental);
  ...
  if (flags) {			/* cost-based algo */
    ...
    pdata->dist = Malloc(nnodes*nnodes,long);
    pdata->heap = Malloc(nnodes,VERTEX);
    pdata->vheap = Malloc(nnodes,int);
    do {
      change = FALSE;
      if (!costgcc_filter(pdata,accum,incremental,0,&change) ||
	  !costgcc_filter(pdata,-accum,incremental,1,&change) ||
	  !costgcc_prune(pdata,w,&change) ||
  ...
  for (i = 0; i < pdata->nvartargets; ++i) {
    ...
    if (live != dvar_value_count(dv))
      dvar_fix_set(dv, gcc_new_domain(pdata, w, 1, live, varvertex),w);
  ...
  if (pdata->cost) {
    int nvars_back = GetSmall(CTagToArg(X(0),1));
...
void SPCDECL 
prolog_fd_gcc_helper MAGIC (HIDDEN_PROTO
			    SP_term_ref State0,
			    SP_term_ref State,
			    SP_term_ref Actions)
{
