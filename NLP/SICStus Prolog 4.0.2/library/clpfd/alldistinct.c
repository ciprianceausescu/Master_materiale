...
#if MULTI_SP_AWARE
#define cmp_asc_label(A1,A2) cmp_asc_label(HIDDEN_ARG, A1,A2)
#define qsort_asc_labelswap(A1,A2,A3) qsort_asc_labelswap(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_labelmed3(A1,A2,A3) qsort_asc_labelmed3(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_label(A1,A2) qsort_asc_label(HIDDEN_ARG, A1,A2)
#if 0 /* MAY COME IN HANDY */
#define cmp_asc_cost(A1,A2) cmp_asc_cost(HIDDEN_ARG, A1,A2)
#define qsort_asc_costswap(A1,A2,A3) qsort_asc_costswap(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_costmed3(A1,A2,A3) qsort_asc_costmed3(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_cost(A1,A2) qsort_asc_cost(HIDDEN_ARG, A1,A2)
#endif /* 0 */
#define remove_neighbor(A1,A2,A3,A4) remove_neighbor(HIDDEN_ARG, A1,A2,A3,A4)
#define propagate_value(A1,A2,A3,A4) propagate_value(HIDDEN_ARG, A1,A2,A3,A4)
#define visit(A1,A2,A3,A4) visit(HIDDEN_ARG, A1,A2,A3,A4)
#define find_sccs(A1,A2,A3) find_sccs(HIDDEN_ARG, A1,A2,A3)
#define single_scc(A1,A2,A3) single_scc(HIDDEN_ARG, A1,A2,A3)
#define compress_edges(A1,A2,A3,A4,A5) compress_edges(HIDDEN_ARG, A1,A2,A3,A4,A5)
#define all_dist_kernel(A1,A2) all_dist_kernel(HIDDEN_ARG, A1,A2)
#define all_dist_filter(A1,A2) all_dist_filter(HIDDEN_ARG, A1,A2)
#define ass_filter(A1,A2) ass_filter(HIDDEN_ARG, A1,A2)
#define remove_edge(A1,A2) remove_edge(HIDDEN_ARG, A1,A2)
#define all_dist_alloc(A1,A2,A3,A4,A5,A6) all_dist_alloc(HIDDEN_ARG, A1,A2,A3,A4,A5,A6)
#define cost_filter(A1,A2,A3,A4) cost_filter(HIDDEN_ARG, A1,A2,A3,A4)
#define cost_based_pruning(A1,A2,A3,A4) cost_based_pruning(HIDDEN_ARG, A1,A2,A3,A4)
#define cons_batch(A1,A2,A3) cons_batch(HIDDEN_ARG, A1,A2,A3)
#define pairing_filter(A1,A2) pairing_filter(HIDDEN_ARG, A1,A2)
...
struct all_dist_data {
  ...
  struct {
    ...
    long *component;		/* Component number */
    long *size;			/* _Saved_ domain size */
  ...
  long *cost;			/* matrix if cost-based, else NULL */
  struct {
    /* the following fields are valid while descending a branch */
    long costbase;		/* cost of all ground items */
    int ntargets;		/* rc indexes range in [0,ntargets-1] */
    int *index_var;		/* rc index to vertex no. */
    int *var_index;		/* vertex no. to rc index */
    long *lbdual[2];		/* min. cost dual soln. */
    long *ubdual[2];		/* max. cost dual soln. */
    int  *lbprimal[2];		/* min. cost primal soln. */
    int  *ubprimal[2];		/* max. cost primal soln. */
    Dvar costvar;
    long *rcost[2];		/* 0=min, 1=max */
    long optcost[2];		/* 0=min, 1=max */
    /* the following fields are volatile */
    int *label[2];		/* for finding aug. path */
    long *cost;
    long *dist;
    long *distrow;		/* points at k'th row of dist */
    int *queue;
    int *index;
    int *heap;
    int *vheap;
    int heapsize;
    long min_regret[2];		/* 0=min, 1=max */
    long max_regret[2];		/* 0=min, 1=max */
    long threshold[2];		/* 0=min, 1=max */
  } rc;
};
...
static SP_BOOL cost_filter MAGIC (HIDDEN_PROTO
...
static void SPCDECL 
all_distinct_daemon MAGIC (HIDDEN_PROTO
			 Argdecl,
			 void *vdata,
			 SP_globref attr_ref,
			 TAGGED global)
...
static int cmp_asc_label MAGIC (HIDDEN_PROTO
			      VERTEX *t1, VERTEX *t2)
{
				  ...
				  VERTEX u, VERTEX v)
{
  ...
  if (dvar_prune_value_t(udv,tv,w)) {
    pdata->pruned = TRUE;
  }
...
#if PROPAGATE
static SP_BOOL propagate_value MAGIC (HIDDEN_PROTO
				   struct all_dist_data *pdata,
				   Argdecl,
				   int nvars, int nvals)
{/* Performs all possible pruning by propagating singletons.
  ...
  while (top>0) {
    ...
    int valtarget = -1;
    VERTEX valvertex = neighbor_of(pdata,dvar_min_t(DVAR(vertex)), &valtarget, vertex);
	...
	VERTEX lvertex = neighbor_of(pdata,dviter_next_value_t(&it), &valtarget, valvertex);
	
	if (lvertex!=vertex)
	  switch (remove_neighbor(pdata,w,lvertex,valvertex)) {
	  case 0: return FALSE;
...
static SP_BOOL augment_path(struct all_dist_data *pdata,
			 VERTEX vertex0,
			 int now)
      ...
      */
{
  ...
  while (p<q) {
    ...
    while (!dviter_empty(&it)) {
      neighbor = neighbor_of(pdata,dviter_next_value_t(&it), &valtarget, vertex);
      ...
      graph until all variable vertices are matched.  */
{
  ...
  for (i=0; i<nvars; i++) {
	...
	rvertex = neighbor_of(pdata,seed,&valtarget,vertex);
	if (exposed(rvertex))
...
fast_find_sccs(struct all_dist_data *pdata, int sccid)
{
  ...
  while (j-top1<numbery) {
    ...
    if (j<numbery && (x==EOL || VISITED(y))) /* node to be ignored */
      ...
      stk.root[0] = y;
      stk.rightmost[0] = y;
      stk.maxx[0] = neighbor_of(pdata,dvar_max_t(DVAR(x)), &valtarget2, x); /* SHAVE? */
      top1 = top2 = 1;
      j++;
    } else if (stk.maxx[top2-1]>=y) { /* start/extend component */
      VERTEX min1 = neighbor_of(pdata,dvar_min_t(DVAR(x)), &valtarget1, x);
      VERTEX max1 = neighbor_of(pdata,dvar_max_t(DVAR(x)), &valtarget2, x);
      ...
      */
{
  VERTEX newvertex=EOL, mate;
   ...
   while (!dviter_empty(&it)) {
     newvertex = neighbor_of(pdata,dviter_next_value_t(&it),&valtarget, mate);
     if (newvertex != vertex) {
...
static SP_BOOL single_scc MAGIC (HIDDEN_PROTO
			      struct all_dist_data *pdata,
			      Argdecl,
			      TAGGED unvisited)
{
  ...
  y = fd_min(unvisited);
  preds = fd_interval(w,y,y);
  valtarget = -1;
  yvertex = neighbor_of(pdata,y,&valtarget, 0);
  unvisited = fd_delete(w,unvisited,y);
  fdset = dvar_set(DVAR(MATE(yvertex)),w);
  while (unvisited!=EmptySet) {
    ...
    if (cmp!=FDI_DISJOINT)
      preds = fd_union_interval(w,preds,y,y);
  }
      ...
      Tarjan's depth-first search based algorithm from 1972. */
{
   ...
   for (i=0; i<nvals; i++) {
     ...
     if (degree==1) {
       ...
       fdcons_add(&cons,w,LABEL(vertex)); /* ORDER */
       nnonground++;
       if (mindeg > degree)
	 mindeg = degree;
     }
   ...
   for (i=0; i<nvars; i++) {
     ...
     if (!VISITED(mate)) {
       TAGGED holes =
	 fd_interval_subtract(w,dvar_min_t(dv), dvar_max_t(dv), dvar_set(dv,w));
 ...
 not_convex:
   if (!single_scc(pdata,w,fdcons_set(&cons))) {
				  ...
				  int nscc)
{
  ...
  long *keyarr = sortarr + nvals;
  
  KEYSORT(vals, nvals, VERTEX, keyarr, nscc, COMPONENT, sortarr);

  while (j<nvals) {
    int comp = COMPONENT(sortarr[j]);
    fdcons_init(&cons);
    for (; j<nvals; j++) {
      VERTEX y = sortarr[j];
      int newcomp = COMPONENT(y);
      if (comp!=newcomp)
	break;
      fdcons_add(&cons,w,LABEL(y));
    }
    for (; i<j; i++) {
      VERTEX x = MATE(sortarr[i]);
      if (x>EOL && dvar_fix_set(DVAR(x), fdcons_set(&cons),w))
	pdata->pruned = TRUE;
    }
  }
}
...
static SP_BOOL all_dist_kernel MAGIC (HIDDEN_PROTO
				   struct all_dist_data *pdata,
				   Argdecl)
{
  FDITER it;
  ...
  if (pdata->nvarkernels>0) {
    for (i=pdata->nvarkernels-1; i>=0; i--) {
      VERTEX v = TVAR(i);
      j += dvar_interval_count(DVAR(v));
    }
    NumstackAlloc(4*j,h);
    for (i=pdata->nvarkernels-1; i>=0; i--) {
      VERTEX v = TVAR(i);
      TAGGED d = dvar_set(DVAR(v),w);
      while (d!=EmptySet) {
    ...
    {
      long maxdeg = 0;
      long nvals = fd_size(vals);
      BOOL cond;
      for (i=pdata->nvarkernels-1; i>=0; i--)
	if (maxdeg < LIVE(TVAR(i)))
	  maxdeg = LIVE(TVAR(i));
      cond = (pdata->nvarkernels>1 && maxdeg > (pdata->nvarkernels*pdata->nvarkernels-nvals)/(pdata->nvarkernels-1));
      printf("# maxdeg=%ld #kernels=%d #values=%ld can eliminate=%s\n", maxdeg, pdata->nvarkernels, nvals, cond ? "TRUE" : "FALSE");
    }
#endif
    fditer_init(&it, vals);
    i = 0;
...
static SP_BOOL all_dist_filter MAGIC (HIDDEN_PROTO
				   struct all_dist_data *pdata,
				   Argdecl)
{
  int i, j, rcall, ncands;
  VERTEX vertex;
  ...
  do {
    ...
    for (i=0; i<pdata->nvarkernels; i++) {
      ...
      vertex = TVAR(i);
      if (GROUND(vertex)) {
	rvertex = neighbor_of(pdata,dvar_min_t(DVAR(vertex)),&valtarget, vertex);
	VISITED(vertex) = 1;
	...
	if (VISITED(vertex)!=0) {
	  VISITED(vertex) = 0;
	  fdcons_add(&cons,w,LABEL(vertex));
	}
      }
      for (i=0; i<pdata->nvarkernels; i++) {
	vertex = TVAR(i);
	...
	if (VISITED(vertex)!=0) {
	  ...
	  rc = dvar_prune_set(dv, fdcons_set(&cons),w);
	  rcall |= rc;
	  if (rc<0)
	    return FALSE;
	  pdata->pruned = TRUE;
  ...
  if (!augment(pdata,pdata->nvarkernels))
    return FALSE;
  ...
  if (slack>0) {
    ...
    for (i=j=0; i<pdata->nvalkernels; i++) {
      vertex = src[i];
      mate = MATE(vertex);
      comp = COMPONENT(vertex);
      COMPONENT(vertex) = 0;
      if (exposed(vertex)) {
	COMPONENT(vertex) = 1;
	fdcons_add(&reach0,w,LABEL(vertex)); /* ORDER */
      } else if (!GROUND(mate)) {
	cands[j++] = vertex;
      }
    }
	...
	vertex = cands[i];
	mate = MATE(vertex);
	if (dvar_compare_set(DVAR(mate),reachable,w)!=FDI_DISJOINT) {
	  COMPONENT(vertex) = 1;
	  COMPONENT(mate) = 1;
	  VISITED(mate) = 1;
	  fdcons_add(&reach0,w,LABEL(vertex)); /* ORDER */
  ...
  if (nunreach>0) {
    int nscc = find_sccs(pdata,w,1);
    
    if (nscc>1)
      compress_edges(pdata,w,pdata->valtarget,pdata->nvalkernels,nscc+2);
	...
	Dvar dv = DVAR(vertex);
	if (dvar_prune_set(dv,scc_values,w)!=0) {
	  pdata->pruned = 1;
	}
      }
    }
...
static void ass_init(struct all_dist_data *pdata,
		     TAGGED tmat)
{
  VERTEX vertex = 0;
  int k;
...
static TAGGED cons_batch MAGIC (HIDDEN_PROTO
				Argdecl,
				int *buf2, int n)
{
  TAGGED t1, t2;
  ...
  case 1:
    ...
    return fd_interval(w,t1,t1);
  case 2:
    t1 = MakeSmall(buf2[0]+1);
    t2 = MakeSmall(buf2[1]+1);
    return fd_pair(w,t1,t2);
  default:
    n2 = n>>1;
    return fd_union_dest(cons_batch(w,buf2,n2),cons_batch(w,buf2+n2,n-n2));
  }
}
...
*/
static SP_BOOL pairing_filter MAGIC (HIDDEN_PROTO
				  struct all_dist_data *pdata,
				  Argdecl)
{
  /* volatile storage: */
  ...
  */
  ...
  for (iter=0; iter<2; iter++) {
		...
		if (max-1>=minv[iter]) {
		  if (type==0 && min-1<=minv[iter])
		    buf2[ap++] = iter ? elt : elt-nxs;
			   ...
			   : (max-1< maxv[iter])) {
		    event.key[ev] = (type==0 ? min-1 : max-1);
		    event.type[ev] = type;
		    event.elt[ev] = elt;
		    event.queue[ev] = ev;
		    ev++;
    ...
    for (i=0; i<nevents; i++) {
      ...
      case 0:			/* add */
	if (dp<nevents) {
	  feasible = fd_subtract(w,feasible,cons_batch(w,buf2+dp, nevents-dp));
	  dp = nevents;
	}
	buf2[ap++] = iter ? elt : elt-nxs;
	break;
      case 2:			/* del */
	if (ap>0) {
	  feasible = fd_union_dest(feasible,cons_batch(w,buf2, ap));
	  ap = 0;
	}
	...
	if (feasible==EmptySet)
	  goto ret;
	dv = DVAR(elt);
	switch (dvar_fix_set(dv,feasible,w)) {
	case -1:
	  goto ret;
	case 0:
	  break;
...
static SP_BOOL ass_filter MAGIC (HIDDEN_PROTO
			      struct all_dist_data *pdata,
			      Argdecl)
{
  int i, nscc;
...
#if PROPAGATE
  if (!propagate_value(pdata,w,nvarkernels,nvalkernels))
    return FALSE;
#endif
  /* Find maximal matching. */
  if (!augment(pdata,nvarkernels))
    return FALSE;
  
  /* Look for SCCs */
  /* In the tight case, can't prune if there is only 1 SCC. */
  /* In the loose case, can't prune if there is only 0 SCC. */
  nscc = find_sccs(pdata,w,1);
  if (nscc > 1) {  
    /* Delete edges */
    compress_edges(pdata,w,pdata->vartarget,nvarkernels,nscc+2);
    compress_edges(pdata,w,pdata->valtarget,nvalkernels,nscc+2);
  }
  return TRUE;
}
...
/* A feasibility check: a necessary condition for circuit/[1,2]
   ...
   */
static SP_BOOL single_circuit(struct all_dist_data *pdata,
			   VERTEX vertex)
{
...
start:
  ...
  while (!dviter_empty(&it)) {
    ...
    child = pdata->tail[neighbor_of(pdata,tv,&valtarget, vertex)-nvars /*LABEL(neighs[j])-1*/];
    m = VISITED(child);
    if (m==0) {
      vstack[top] = vertex;
      tstack[top+1] = tv;
...
static SP_BOOL
single_circuit_check(struct all_dist_data *pdata)
{
  int nvars = pdata->nvars;
  int maxdegree = 0;
  ...
  return single_circuit(pdata,bestv);
}
		      ...
		      int flags) /* 0x1 = circuit, 0x2 = cost matrix */
{
  ...
  pdata->flags = flags;
    ...
    if (!(flags & 0x2)) {
      ...
      pdata->cost = (long *)ptr;
      ptr += nvars*nvars*sizeof(long);
      for (i=0; i<2; i++) {
	pdata->rc.rcost[i] = (long *)ptr;
	ptr += nvars*nvars*sizeof(long);
	pdata->rc.lbdual[i] = (long *)ptr;
	ptr += nvars*sizeof(long);
	pdata->rc.ubdual[i] = (long *)ptr;
	ptr += nvars*sizeof(long);
      }
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    nvars2 = nvars*nvars;
    if (!AreSmall(fd_min(all),fd_max(all)))
      nvals = nvars2;
    else
      nvals = fd_size(all);
    fd.gdata = pdata = all_dist_alloc(w,nvars,(nvals<nvars2 ? nvals : nvars2),nvars,handle,flags);
    pdata->stamp = state_stamp-1; /* DON'T trust initially */
    if (fd_compare_interval(all, MakeSmall(1), MakeSmall(nvars))==FDI_EQUAL)
    ...
    for (i=0; i<nvars; i++) {
      DerefCar(telt,tlvec);
      DerefCdr(tlvec,tlvec);
      get_var_and_attr(telt,RefAttr(i));
      if (pdata->enqueue_code) {
	dvar_init(DVAR(i), RefAttr(i), RefVar(i));
	dvar_attach_daemon(w, DVAR(i), pdata, X(1), f);
      }
    }
  ...
  if (!pdata->incremental) {
    ...
    DerefArg(telt,X(0),2);
    pdata->nvartargets = pdata->nvars-GetSmall(telt);
    DerefArg(telt,X(0),3);
    pdata->nvaltargets = pdata->nvals-GetSmall(telt);
    if (pdata->density>0 && pdata->nvaltargets>nvals)
      qsort_asc_label(pdata->valtarget,pdata->nvaltargets);
  }
  ...
  if (pdata->pruned) {
    for (i=pdata->nvartargets-1; i>=0; i--) {
      ...
      dvar_pruning_done(w,DVAR(vertex));
    }
    for (i=pdata->nvartargets-1; i>=0; i--) {
      VERTEX vertex = TVAR(i);
    
      dvar_export(w,DVAR(vertex));
    }
  }
  contract_vars(pdata);
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    fd.gdata = pdata = all_dist_alloc(w,nvars,nvals,nrefs,handle,flags);
    pdata->stamp = state_stamp-1; /* DON'T trust initially */
    DerefArg(tlvec,X(0),1);
    ...
    for (i=0; i<nvars; i++) {
      ...
      get_var_and_attr(telt,RefAttr(nvars+i));
    }
    DerefArg(telt,X(0),5);
    if (!IsAtomic(telt)) {
      get_var_and_attr(telt,RefAttr(nrefs-1));
  ...
  if (!pdata->incremental) {
    ...
    if (pdata->nvartargets < nvars1) {
      ...
      qsort_asc_label(pdata->vartarget,nvars1);
    }
    if (pdata->nvaltargets < nvals1) {
      pdata->nvaltargets = nvals1;
      qsort_asc_label(pdata->valtarget,nvals1);
    }
  }
  ...
  pdata->nvarkernels = pdata->nvartargets;
  pdata->nvalkernels = pdata->nvaltargets;
  if (!pairing_filter(pdata,w))
    goto ret;
  if (!pdata->cost ? !ass_filter(pdata,w) :
      !cost_filter(pdata,w,GetSmall(CTagToArg(X(0),6)),flags))
    goto ret;
  ...
  if (pdata->pruned) {
    ...
    if (pdata->cost)
      dvar_pruning_done(w,pdata->rc.costvar);
    for (i=pdata->nvartargets-1; i>=0; i--)
      for (j=0; j<2; j++) {
	VERTEX vertex = j==0 ? TVAR(i) : TVAL(i);
    ...
    if (pdata->cost)
      dvar_export(w,pdata->rc.costvar);
  }
  if (pdata->cost) {
    long costbase = GetSmall(CTagToArg(X(0),6));
...
#if 0 /* MAY COME IN HANDY */
static int cmp_asc_cost MAGIC (HIDDEN_PROTO
			       int *t1, int *t2)
{
  struct all_dist_data *pdata = fd.gdata;
  long c1, c2;
...
static void decrease_key(struct all_dist_data *pdata,int v, long value)
{
...
static SP_BOOL
cost_based_filter(struct all_dist_data *pdata,
		  SP_BOOL b /* 0=min, 1=max */)
{
  int n = pdata->rc.ntargets;
  int nvars = pdata->nvars;
  int i, j, nextj, u, v, card;
  long max_regret, min_regret;
  long **dual = (b==0 ? pdata->rc.lbdual : pdata->rc.ubdual);
  int  **primal = (b==0 ? pdata->rc.lbprimal : pdata->rc.ubprimal);
  long *cost = pdata->rc.cost;
  long *rcost = pdata->rc.rcost[b];
		    ...
		    int b)
{
  ...
  long *lstack = (long *)pdata->dist_stack;
  long *regret = pdata->rc.rcost[b];
  int n = pdata->rc.ntargets;
...
#if 0 /* MAY COME IN HANDY */
/* Ripped off from visit() */
static int
cost_visit(struct all_dist_data *pdata,
	   int vertex,
	   long max_regret,
	   int *map,
	   int b)
{
  ...
  for (i=0; i<n; i++) {
    ...
    if (newvertex != vertex) {
      m = (visited[newvertex] == 0) ? 
	cost_visit(pdata,newvertex,max_regret,map,b) :
	visited[newvertex];
      if (m < min)
	min = m;
    }
...
static SP_BOOL
cost_based_pruning MAGIC (HIDDEN_PROTO
			  struct all_dist_data *pdata,
			  Argdecl,
			  int flags,
			  SP_BOOL *change)
{
  ...
  if (pdata->rc.max_regret[0] > pdata->rc.threshold[0] ||
      ...
      cost_has_hole) {
    long *threshold = pdata->rc.threshold;
    long **rcost = pdata->rc.rcost;
    long regret[2];
	  ...
	  if (regret[0] > threshold[0] ||
	      ...
	      (cost_has_hole && regret[0]+regret[1]>0 &&
	       dvar_compare_interval_l(pdata->rc.costvar,
				       ...
				       pdata->rc.optcost[1] - regret[1],w) ==
	       FDI_DISJOINT))
	    fdcons_add(&cons,w,LABEL(pdata->rc.index_var[j+n]));
	}
	if (fdcons_size(&cons)>0) {
	  if (dvar_prune_set(DVAR(v),fdcons_set(&cons),w) == -1)
	    return FALSE;
	  pruned = TRUE;
	}
      }
	  ...
	  if (regret[0] > threshold[0] ||
	      ...
	      (cost_has_hole && regret[0]+regret[1]>0 &&
	       dvar_compare_interval_l(pdata->rc.costvar,
				       ...
				       pdata->rc.optcost[1] - regret[1],w) ==
	       FDI_DISJOINT))
	    fdcons_add(&cons,w,LABEL(pdata->rc.index_var[i]));
	}
	if (fdcons_size(&cons)>0) {
	  if (dvar_prune_set(DVAR(v),fdcons_set(&cons),w) == -1)
	    return FALSE;
  ...
  if (flags&1)
    ...
    for (b=0; b<2; b++) {
      int nn = n*n;
      long *regarr = Malloc(nn,long);
      long *regret = pdata->rc.rcost[b];
      long threshold = pdata->rc.threshold[b];
      int nregrets = 0;
      int inf = -1, mid, sup;
      long optreg = 0;
	...
	for (i=n-1; i>=0; i--)
	  if (visited[i] == 0)
	    cost_visit(pdata,i,regret0,map,b);
	...
	if (regret[i]<=threshold)
	  regarr[j++] = regret[i];
      nregrets = j;
      qsort_asc_long(regarr, nregrets);
	...
	long mstcost = pdata->rc.costbase;
	int *set = (int *)head;
	...
	nedges = j;
	qsort_asc_cost(indexarr, nedges);
			       ...
			       int flags)
{
  ...
  if (pdata->incremental)
    ...
    for (i=0, n=0; i<nvartargets; i++) {
      VERTEX vertex = TVAR(i);
      pdata->rc.var_index[vertex] = n;
      pdata->rc.index_var[n++] = vertex;
    }
  ...
  pdata->rc.cost = Malloc(n*n,long); /* pos. or neg. submatrix */
  pdata->rc.dist = Malloc(n*n,long); /* distance matrix */
  ...
  do {
    change = FALSE;
    if (!cost_based_filter(pdata,0) ||
	!cost_based_filter(pdata,1) ||
	!cost_based_pruning(pdata,w,flags,&change))
      ok = FALSE;
  } while (ok && change);
...
void SPCDECL 
prolog_fd_assignment_helper MAGIC (HIDDEN_PROTO
				   SP_term_ref State0,
				   SP_term_ref State,
				   SP_term_ref Actions)
{
