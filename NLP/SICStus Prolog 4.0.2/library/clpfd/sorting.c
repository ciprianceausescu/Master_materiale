...
#if MULTI_SP_AWARE
#define cmp_asc_min(A1,A2) cmp_asc_min(HIDDEN_ARG, A1,A2)
#define qsort_asc_minswap(A1,A2,A3) qsort_asc_minswap(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_minmed3(A1,A2,A3) qsort_asc_minmed3(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_min(A1,A2) qsort_asc_min(HIDDEN_ARG, A1,A2)
#define cmp_asc_max(A1,A2) cmp_asc_max(HIDDEN_ARG, A1,A2)
#define qsort_asc_maxswap(A1,A2,A3) qsort_asc_maxswap(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_maxmed3(A1,A2,A3) qsort_asc_maxmed3(HIDDEN_ARG, A1,A2,A3)
#define qsort_asc_max(A1,A2) qsort_asc_max(HIDDEN_ARG, A1,A2)
#define normalize(A1) normalize(HIDDEN_ARG, A1)
...
static int 
cmp_asc_min MAGIC (HIDDEN_PROTO
		   int *x1, int *x2)
{
...
static int 
cmp_asc_max MAGIC (HIDDEN_PROTO
		   int *x1, int *x2)
{
  struct sorting_data *pdata = fd.gdata;
		 ...
		 struct sorting_data *pdata)
{
  ...
  while (donormal) {
    ...
    qsort_asc_min(pdata->sortmin, ntargets);
    ...
    qsort_asc_max(pdata->sortmax, ntargets);
    /* scan Xs and Ps in sync */
...
findscc_p(struct sorting_data *pdata)
{
  ...
  while (j-top1<ntargets) {
    ...
    if (top2==0) {	/* start 1st component */
      ...
      pdata->stk.root[0] = y;
      pdata->stk.rightmost[0] = y;
      pdata->stk.maxx[0] = PMAX(x);
      top1 = top2 = 1;
      j++;
    } else if (j<ntargets &&
	       pdata->stk.maxx[top2-1]>=y+1) { /* start/extend component */
      int x = YMATE(y);
      int max1 = PMAX(x);
...
*/
void SPCDECL
prolog_fd_sorting MAGIC (HIDDEN_PROTO
			 SP_term_ref State0,
			 SP_term_ref State,
			 SP_term_ref Actions)
{
  ...
  long state_stamp;
  int i, j, nvars, ntargets, total_size;
  struct sorting_data *pdata;
  SP_BOOL committed;
  char *ptr;
  WAMENV;
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
	...
	DerefCar(telt,tvec);
	DerefCdr(tvec,tvec);
	get_var_and_attr(telt,RefAttr(j+base));
      }
    }
  ...
  /* RESUME HERE */
  ptr = fd_malloc(nvars*(sizeof(TAGGED) + 11*sizeof(int)));
  ...
  for (i=0; i<ntargets; i++) {
    dvar_export(w,XVAR(XV(i)));
    dvar_export(w,PVAR(XV(i)));
    dvar_export(w,YVAR(YV(i)));
  }
  if (ntargets>0) {
