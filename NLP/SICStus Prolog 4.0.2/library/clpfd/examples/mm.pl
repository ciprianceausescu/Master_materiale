...
set_black_constraint([O1|Old], [B|Bs], [N1|New]) :-
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 % set_white_constraint/4 enforces:
 % Blacks+Whites = sum(min(NewCount,OldCount)) for every element in the new guess and
...
set_white_constraint(Occs, Blacks, Whites, New) :-
	colors(NC),
	new_occs(0, NC, Occs, NewOccs, Terms),
	global_cardinality(New, NewOccs, [consistency(value)]),
