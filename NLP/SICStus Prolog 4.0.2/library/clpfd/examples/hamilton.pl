...
hc(static) :-
	...
	make_domains(Clumps, Path),
	circuit(Path),
	get_vars(Nodes, Vars, Tree),
	labeling([enum], Vars),
	writeq(Path),
	nl.

get_vars([], [], _).
get_vars([N|Ns], [V|Vs], Tree) :-
	get_label(N, Tree, V),
	get_vars(Ns, Vs, Tree).

make_domains([], _).
make_domains([_-Ns|Clumps], [P|Path]) :-
	list_to_fdset(Ns, Set),
	P in_set Set,
	make_domains(Clumps, Path).
...
vertex_order(VertNeibs1) --> 
	{degrees_and_ids(VertNeibs1, DI1)},
	{keysort(DI1, DI2)},
	{DI2 = [_-Vertex|_]},
...
degrees_and_ids([], []).
degrees_and_ids([V-Ns|Clumps], [K-V|KVs]) :-
	length(Ns, K),
	degrees_and_ids(Clumps, KVs).
