...
dna(seed, NbOctos) :-
	system(NbOctos, Octos),
	append(Octos, Vars),
	seed(0, NbOctos, Octos),
	labeling([], Vars),
	display_each(Octos).

lab_shave([]).
lab_shave([Octo|Octos]) :-
	shave(Octo),
	labeling([], Octo), !,
	lab_shave(Octos).
...
display_each([Octo|Octos]) :-
	display_octo(Octo, String),
	format('~s\n', [String]),
	display_each(Octos).

display_octo([], []).
display_octo([B|Bs], [C|Cs]) :-
	acgt_map(B, C),
	display_octo(Bs, Cs).
...
system(NbOctos, Octos) :-
	...
	domains(Octos),
	% /*A2*/lex_chain(Octos, [op(#<),among(4,4,[0,1])]), % slower, but slightly fewer bktr
	/*A1*/halfcg_each(Octos),	% faster, but slighly more bktr
	/*A1*/octos_pentas(Octos, Pentas),
	/*A1*/lex_chain(Pentas, [op(#<)]), % saves runtime and backtracks
	/*B1*/all_compl_local(Octos).
	% /*B2*/all_compl_global(Octos). % _more_ bktr

octos_pentas([], []).
octos_pentas([[A,B,C,D,E,_,_,_]|Os], [[A,B,C,D,E]|Ps]) :-
	octos_pentas(Os, Ps).
