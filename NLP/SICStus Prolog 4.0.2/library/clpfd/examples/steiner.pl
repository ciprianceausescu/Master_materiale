...
steiner(dual, _Opt, N, Consistency) :-
	...
	format_rows(Triples, '~t~d~+~t~d~+~t~d~+\n').
...
pair_constraints(Triples, N, Consistency) :-
	triples_codes(Triples, N, Codes, Consistency),
	all_distinct(Codes, [consistency(Consistency)]).

triples_codes([], _, [], _).
triples_codes([[A,B,C]|Ts], N, [AB,AC,BC|Codes], Consistency) :-
	scalar_product([N,1], [A,B], #=, AB, [consistency(Consistency)]),
	scalar_product([N,1], [A,C], #=, AC, [consistency(Consistency)]),
	scalar_product([N,1], [B,C], #=, BC, [consistency(Consistency)]),
	triples_codes(Ts, N, Codes, Consistency).
