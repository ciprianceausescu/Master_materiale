...
#define qsort_desc_dur2d0swap(A1,A2,A3) qsort_desc_dur2d0swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_est2d0swap(A1,A2,A3) qsort_asc_est2d0swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_desc_est2d0swap(A1,A2,A3) qsort_desc_est2d0swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_lct2d0swap(A1,A2,A3) qsort_asc_lct2d0swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_est2d1swap(A1,A2,A3) qsort_asc_est2d1swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_desc_est2d1swap(A1,A2,A3) qsort_desc_est2d1swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_lct2d1swap(A1,A2,A3) qsort_asc_lct2d1swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_start0swap(A1,A2,A3) qsort_asc_start0swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_end0swap(A1,A2,A3) qsort_asc_end0swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_start1swap(A1,A2,A3) qsort_asc_start1swap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_asc_end1swap(A1,A2,A3) qsort_asc_end1swap(HIDDEN_ARG, A1,A2,A3)
...
#define profile_init_complement(A1,A2,A3) profile_init_complement(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_rectanglesswap(A1,A2,A3) qsort_rectanglesswap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_events_increasing_orderswap(A1,A2,A3) qsort_events_increasing_orderswap(HIDDEN_ARG, A1,A2,A3)
...
#define qsort_events_decreasing_orderswap(A1,A2,A3) qsort_events_decreasing_orderswap(HIDDEN_ARG, A1,A2,A3)
...
#define disjoint2_filter(A1,A2,A3,A4,A5,A6) disjoint2_filter(HIDDEN_ARG, A1,A2,A3,A4,A5,A6)
#define disjoint2_check(A1,A2,A3) disjoint2_check(HIDDEN_ARG, A1,A2,A3)
#define decompose2d(A1,A2,A3,A4) decompose2d(HIDDEN_ARG, A1,A2,A3,A4)
...
struct disjoint2_data {
  ...
  struct {
    ...
    long *pruned_at[2];		/* >0 if pruned, volatile */
  ...
  struct {
    ITEM *item;			/* , volatile */
    long *amount[2];		/* virtual shift amount, volatile */
  } shifted_item;		/* each [nshifted_items] */
  struct {
    ITEM *item;			/* , volatile */
    long *start[2];		/* inclusive start of forbidden region, volatile */
    long *end[2];		/* inclusive end of forbidden region, volatile */
    long *min_start[2];		/* mimimal start among this and later end_event, volatile */
    long *max_end[2];		/* maximal end among this and earlier start_event, volatile */
    long *min_start_t[2];	/* mimimal start among this and later end_event_t, volatile */
    long *max_end_t[2];		/* maximal end among this and earlier start_event_t, volatile */
				  ...
				  ITEM *t1, ITEM *t2)
{
  ...
  long dur1 = DUR2D(*t1,0);
  long dur2 = DUR2D(*t2,0);
...
{
  struct disjoint2_data *pdata = fd.gdata;
  long lct1 = LCT2D(*t1,0);
  long lct2 = LCT2D(*t2,0);
...
{
  struct disjoint2_data *pdata = fd.gdata;
  long lct1 = LCT2D(*t1,1);
  long lct2 = LCT2D(*t2,1);
...
{
  struct disjoint2_data *pdata = fd.gdata;
  long val1 = pdata->event.start[0][(*t1)];
  long val2 = pdata->event.start[0][(*t2)];
...
{
  struct disjoint2_data *pdata = fd.gdata;
  long val1 = pdata->event.end[0][(*t1)];
  long val2 = pdata->event.end[0][(*t2)];
...
{
  struct disjoint2_data *pdata = fd.gdata;
  long val1 = pdata->event.start[1][(*t1)];
  long val2 = pdata->event.start[1][(*t2)];
...
{
  struct disjoint2_data *pdata = fd.gdata;
  long val1 = pdata->event.end[1][(*t1)];
  long val2 = pdata->event.end[1][(*t2)];
					      ...
					      TAGGED head, TAGGED tail)
{
  if (tail==EmptySet)
    ...
    return profile_cons(w,GetSmall(RangeMax(head))+1,
			GetSmall(RangeMin(head1)),
			1,
			profile_init_complement(w,head1,tail1));
  }
}

static SP_BOOL prune_item2d MAGIC (HIDDEN_PROTO
				ITEM sv,
...
-- return val+1.
----------------------------------------------------------------------*/
static long nextval_origin(struct disjoint2_data *pdata,
			   int                    rect ,
			   int                    dim  ,
			   long                   val  )
{
  if (ISTARGET(rect)) {
    long succ = dvar_successor_l(DVAR2D(rect,dim), val);

    return (succ<CLPFD_MAXINT ? succ : val+1);
  } else {
    long max = LST2D(rect,dim);
  ...
  struct disjoint2_data *pdata          )   /* pointer to rectangles frame */
{
  ...
  if ((pdata->flags&0x48)==0x40 &&          /* synchronization(true),margins(false) */
    ...
    &&  (nfree_var > 1)) {                  /* and if there exist at least two free variables */
      synchro_table = fd_malloc(sizeof(struct synchro_frame));
      ...
      synchro_table->ori_depth  = fd_malloc(size_table);
      synchro_table->len_depth  = fd_malloc(size_table);
      synchro_table->sort_task  = fd_malloc(size_table);
      synchro_table->next_task  = fd_malloc(size_table);
      synchro_table->last_part  = fd_malloc(size_table);
      synchro_table->next_part  = fd_malloc(size_table);
      synchro_table->hint       = fd_malloc(size_table);
...
----------------------------------------------------------------------*/
static SP_BOOL sweep_synchronization MAGIC (HIDDEN_PROTO
  struct disjoint2_data *pdata          ,   /* pointer to rectangles frame */
  int                    task           ,   /* index of the first rectangle of current task */
  SP_BOOL                  *change         )
{
...
-- and TRUE otherwise.
----------------------------------------------------------------------*/
static SP_BOOL wake_sweep_synchro MAGIC (HIDDEN_PROTO
  struct disjoint2_data *pdata            , /* pointer to rectangles frame */
                                            ...
                                            /* return FALSE if did find a contradiction */
{
  ...
  matching_table = fd_malloc(sizeof(struct matching_frame));
  ...
  assign         = fd_malloc(size_int_max_nb_part); assign--        ;
  hidden         = fd_malloc(size_int_nb_resource); hidden--        ;
  rmate          = fd_malloc(size_int_max_nb_part); rmate--         ;
  ymate          = fd_malloc(size_int_nb_resource); ymate--         ;
  mark           = fd_malloc(size_int_nb_resource); mark--          ;
  father         = fd_malloc(size_int_nb_resource); father--        ;
  next_free_var  = fd_malloc(size_int_max_nb_part); next_free_var-- ;
  prev_free_var  = fd_malloc(size_int_max_nb_part); prev_free_var-- ;
  queue_of_nodes = fd_malloc(size_int_max_nb_part); queue_of_nodes--;
  sort_event     = fd_malloc(size_event_int      );
  type_event1    = fd_malloc(size_event_int      );
  date_event1    = fd_malloc(size_event_long     );
  ress_event1    = fd_malloc(size_event_long     );
  type_event2    = fd_malloc(size_event_int      );
  date_event2    = fd_malloc(size_event_long     );
  ress_event2    = fd_malloc(size_event_long     );
  count_forbid   = fd_malloc(size_int_nb_resource); count_forbid--  ;
...
static SP_BOOL prune_before2d MAGIC (HIDDEN_PROTO
...
*/
static SP_BOOL edge_finding2d_up MAGIC (HIDDEN_PROTO
				     SP_BOOL *change,
				     int dim)
{
  struct disjoint2_data *pdata = fd.gdata;
...
*/
static SP_BOOL edge_finding2d_down MAGIC (HIDDEN_PROTO
				       SP_BOOL *change,
				       int dim)
{
  struct disjoint2_data *pdata = fd.gdata;
...
static SP_BOOL feasible_minwit MAGIC (HIDDEN_PROTO
...
static SP_BOOL feasible_maxwit MAGIC (HIDDEN_PROTO
...
*/
static SP_BOOL less_constrained2d MAGIC (HIDDEN_PROTO
...
*/
static SP_BOOL dominates2d MAGIC (HIDDEN_PROTO
			       ITEM r1, ITEM r2, SP_BOOL rightp, int k)
{
  struct disjoint2_data *pdata = fd.gdata;
				   ...
				   SP_BOOL *change)
{
  ...
  for (i=0; i<ntargets; i++) {
    ...
    for (k=0; k<2; k++) {
      ...
      int js, je, mid, sup;
      TAGGED tdom = dvar_set(DVAR2D(titem,1-k),w);
	...
	fd.profile = profile_init_complement(w,CTagToCar(tdom),CTagToCdr(tdom));
	current = est0 = est[k];
	while (current <= lst[k]) {
	  if (next_start <= current) {
	    EVENT ev = pdata->start_event[k][js];
	    /* trace_orig(titem,ev,current,k,1,1); */
	    fd.profile = profile_update(w,fd.profile,
...
*/
static SP_BOOL disjoint2_check MAGIC (HIDDEN_PROTO
				   Argdecl,
				   int nsources, SP_BOOL incremental)
{
  struct disjoint2_data *pdata = fd.gdata;
  SP_BOOL ok = FALSE;
  int i, k, js, je;
  ...
  while (js<nsources || je<nsources) {
	...
	goto fail;
      fd.profile = profile_update(w,fd.profile,ymin,ymax,1);
      js++;
      next_start = js<nsources ? pdata->event.start[0][(queues[js])] : CLPFD_MAXINT;
    } else {
      EVENT ev = queuee[je];
...
static int collect_source2d MAGIC (HIDDEN_PROTO SP_BOOL filtering)
{
  struct disjoint2_data *pdata = fd.gdata;
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    total_size =
      ...
      ((11*nitems + 5*nmargs + nmargs_aligned)*sizeof(long));
    ...
    for (i=0; i<nitems; i++) {
      ...
      j = 0;
      for (k=0; k<2; k++) {
	get_var_and_attr(item+WD(j),ref);
	get_var_and_attr(item+WD(j+2),ref+2);
	ref += 4;
	j += 4;
    ...
    CTagToArg(X(0),3) = atom_nil; /* [MC] 3.12: free for GC */
				...
				/* RESUME HERE */
  ptr = fd_malloc((8*nitems + 25*nshifted_items)*sizeof(long));
  ...
  for (k=0; k<2; k++) {
    ...
    pdata->item.pruned_at[k] = (long *)ptr; /* volatile */
  ...
  for (k=0; k<2; k++) {
    pdata->shifted_item.amount[k] = (long *)ptr; /* volatile */
  ...
  for (k=0; k<2; k++) {
    pdata->event.start[k] = (long *)ptr; /* volatile */
    ptr = (char *)(pdata->event.start[k]+nshifted_items);
    pdata->event.end[k] = (long *)ptr; /* volatile */
    ptr = (char *)(pdata->event.end[k]+nshifted_items);
    pdata->event.min_start[k] = (long *)ptr; /* volatile */
    ptr = (char *)(pdata->event.min_start[k]+nshifted_items);
    pdata->event.max_end[k] = (long *)ptr; /* volatile */
    ptr = (char *)(pdata->event.max_end[k]+nshifted_items);
    pdata->event.min_start_t[k] = (long *)ptr; /* volatile */
    ptr = (char *)(pdata->event.min_start_t[k]+nshifted_items);
    pdata->event.max_end_t[k] = (long *)ptr; /* volatile */
  ...
  if (flags & 0x6) {
    ...
    for (i=0; i<pdata->ntargets; i++) {
      ...
      if (flags & 0x8)
	hits = extend_forbidden_region2d(it, it, start, end);
  ...
  if (!(incremental && (flags & 0x20)) && /* not in the incremental case */
    ...
    if (j>0 && !disjoint2_check(w,j,incremental))
      goto ret;
  }
#endif
  ...
  if (pruned1==TERMIN) {
    ...
    if (tix>0) {
      ...
      if (j>0)
	if (!disjoint2_filter(w,j,target0,tix,incremental,&change))
	  goto ret;
      pdata->iteration++;
    }
  }
  ...
  for (; change; pdata->iteration++) {
    ...
    if (!(flags & 0x8)) { /* no margins */
      j = collect_source2d(TRUE);
      if (j>0)
	if (!disjoint2_filter(w,j,pdata->target,pdata->ntargets,
	...
	j = collect_source2d(TRUE);
	if (j>0)
	  if (!disjoint2_filter(w,j,pdata->target+ti,1,
				(incremental || pdata->iteration>1),
				&change))
	    goto ret;
      }
  ...
  decompose2d(w,j,nactive_items,(incremental && (flags & 0x20)));
