...
process_files([File|Files]) :- !,
    ...
    process_files(Files).
process_files(File) :-
    absolute_file_name(File, Path, [file_type(source)]), % [PM] 4.0.2 add source extensions (SPRM 9763)
    (	processed(Path) ->
	    true
...
skip_past_culprit([File|Rest], Culprit) :-
    absolute_file_name(File, Culprit, [file_type(source)]), % [PM] 4.0.2 add source extensions (SPRM 9763)
    !,
    go(Rest).
...
handle_switches([S|Ss], Fs) :-
    ...
    ;	looks_like_a_switch(S) ->
	    format('Unknown switch ~w~n', [S]), % [PM] Should use user_error!
	    usage,
	    error_exit
    ;	Fs = [S|Ss]
    ).
