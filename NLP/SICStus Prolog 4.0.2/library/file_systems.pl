...
%@  predicates in @code{library(directory)}
...
rename_file(OldName, NewName) :-
	Goal = rename_file(OldName, NewName),
        prolog:rename_file(OldName, NewName, [], Goal).
...
rename_directory(OldName, NewName) :-
	Goal = rename_directory(OldName, NewName),
        prolog:rename_file(OldName, NewName, [directory(true)], Goal).
...
do_delete_file(OldName, Goal) :-
        prolog:delete_file(OldName, [], Goal).
...
delete_directory(Directory, Options) :-
	...
	delete_directory_options(Options, Goal, error, IfNonempty),
	do_delete_directory(Directory, IfNonempty, Goal).

delete_directory_options([], _, Flag, Flag).
delete_directory_options([O|Os], Goal, Flag1, Flag3) :-
	delete_directory_option(O, Goal, Flag1, Flag2),
	delete_directory_options(Os, Goal, Flag2, Flag3).

delete_directory_option(if_nonempty(Flag), Goal, _, Flag) :- !,
	must_be(Flag, oneof([delete,error,ignore,fail]), Goal, 2).
delete_directory_option(Option, Goal, _, _) :-
	illarg(domain(term,option), Goal, 2, Option).
...
do_delete_directory(Directory, IfNonempty, Goal) :-
        ...
        ( prolog:delete_file(Directory, [directory(true), if_nonempty(DFIfNonempty)], Goal) ->
           true
...
do_delete_directory_nonempty(delete, Directory, Goal) :-
	...
	do_delete_files(FilePairs, Goal),
        directory_members_of_directory1(Directory, '*', DirectoryPairs, Goal),
	do_delete_directories(DirectoryPairs, Goal),
	do_delete_directory(Directory, error, Goal).

do_delete_files([], _).
do_delete_files([_-File|Pairs], Goal) :-
	do_delete_file(File, Goal),
	do_delete_files(Pairs, Goal).

do_delete_directories([], _).
do_delete_directories([_-Directory|Pairs], Goal) :-
	do_delete_directory(Directory, delete, Goal),
	do_delete_directories(Pairs, Goal).
...
make_directory(Directory) :-
        ...
        prolog:create_directory(Directory, [], Goal).

make_directory(Directory, Options) :-
        Goal = make_directory(Directory, Options),
        prolog:create_directory(Directory, Options, Goal).
...
file_property2(Thing, Property, ThingType, PropertyType, Value, Goal) :-
        ...
        ; prolog:absolute_file_name(Thing, AbsPath, [file_type(directory),access(exist),file_errors(fail)], Goal, 1) -> % is it a director?
           ThingType = directory
	...
	;   illarg(domain(term,property), Goal, 2)
        ...
        prolog:file_property(AbsPath, Selector, Goal),
...
current_directory(OldDirectory, NewDirectory) :-
	...
	prolog:absolute_file_name(NewDirectory, Abs, [], Goal, 1),
...
current_directory(OldDirectory, NewDirectory) :-
	...
	( OldDirectory == NewDirectory ->       % special case
           ...
           true
        ; prolog:absolute_file_name(NewDirectory, NewDirectoryAbs, [file_type(directory),access(exist)], Goal, 1),
