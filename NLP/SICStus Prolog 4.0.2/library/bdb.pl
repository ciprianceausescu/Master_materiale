...
%@  This means that multiple processes can open the same databas, but
...
%@  subject to @code{absolute_file_name/3} conversion.
%@  
...
%% [PD] 4.0.0beta1 All file ops should use SICStus built in file routines.
...
%@  database.  @var{DBName} is not subject to
%@  @code{absolute_file_name/3} conversion.
%@  
...
db_open(DBName, Mode, SpecList, Options, DBRef) :-
	...
	must_be(Options, proper_list(callable), ErrGoal, 4),
...
normalise_spec_list(SpecList, NSpecList) :-
	spec_keys(SpecList, KSpecList),
	keysort(KSpecList, KSSpecList),
	drop_keys(KSSpecList, NSpecList).

spec_keys([], []).
spec_keys([S|Spec], [K-S|KSpec]) :-
	spec_key(S, K),
	spec_keys(Spec, KSpec).
...
db_error(Ctxt, ErrorCode) :-
	...
	illarg(system(berkeley_db(Ctxt,Err)), 0, 0, 0). % TODO: msgs.pl
...
must_be_valid_mode(Mode, ErrGoal, ArgNo) :-
	...
	;   illarg(domain(atom,one_of([read,update,enumerate])),
			  ErrGoal, ArgNo, Mode)
	).
...
must_be_valid_term(Term, ErrGoal, ArgNo) :-
	(   callable(Term) -> true
	;   illarg(type(callable), ErrGoal, ArgNo, Term)
	).

must_be_integer(Term, ErrGoal, ArgNo) :-
	(   integer(Term) -> true
	;   illarg(type(integer), ErrGoal, ArgNo, Term)
	).
...
%%% fastrw backward compatibility wrapper %%%%%%%%%5
...
from_fastrw_res(0, _Culprit) :- !.
from_fastrw_res(Code, Culprit) :-
   illarg(system(berkeley_db(Culprit,error(Code))), 0, 0, 0). % TODO: msgs.pl
...
export1(DBName, Options, ExportFile) :-
    ...
    db_open_options(Options, none, EnvRef, 0, _CacheSize),
...
import_properties(properties(PropertyList),EnvProp,DBName,SpecList,DaTime,HostName,Version) :-
...
import_property(environment(EnvProp),EnvProp,_DBName,_SpecList,_DaTime,_HostName,_Version).
import_property(database(DBName),_EnvProp,DBName,_SpecList,_DaTime,_HostName,_Version).
import_property(speclist(SpecList),_EnvProp,_DBName,SpecList,_DaTime,_HostName,_Version).
%% This one is (not yet) written by db_export/[2,3]
import_property(timestamp(DaTime),_EnvProp,_DBName,_SpecList,DaTime,_HostName,_Version).
%% This one is (not yet) written by db_export/[2,3]
import_property(hostname(HostName),_EnvProp,_DBName,_SpecList,_DaTime,HostName,_Version).
import_property(version(Version),_EnvProp,_DBName,_SpecList,_DaTime,_HostName,Version).
% Ignore unknown properties
import_property(_, _EnvProp,_DBName,_SpecList,_DaTime,_HostName,_Version).
...
import_store(_DBRef, end_of_file).
import_store(DBRef ,term(Term)) :-
    db_store(DBRef, Term, _TermRef),
    !.
import_store(_ ,ImportTerm) :-
...
foreign_resource(bdb,
  [
   ...
   db_term_hash,ixkeys,decode_error,db_normal_path,sync_db
...
%@  Otherwise, we choose the the spec with the most @code{+} postitions in
...
%@  (@pxref{Berkeley DB}), the standard Berkeley DB migration tools will not
