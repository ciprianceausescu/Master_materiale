...
:- multifile user:clause_expansion/6.

user:clause_expansion(Clause0, Lay0, Ids, Clause0, Lay0, [detcheck|Ids]) :-
    nonmember(detcheck, Ids),
    prolog_load_context(module, Module),
    prolog_load_context(file, File),
    Module \== detcheck,			% don't expand myself!
    !,
    prolog:clause_expansion_loop_internal(Clause0, Lay0, [detcheck|Ids], Clauses, _),
    handle(Clauses, Module, File).
