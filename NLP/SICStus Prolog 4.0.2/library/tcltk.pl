...
call_from_tcl(StreamCode, Interp) :-
	...
	add_result(Vars, Interp).

add_result([], _Interp).
add_result([(Var = Val)|T], Interp) :-
	'$tcl_add_result'(Interp, Var, Val),
	add_result(T, Interp).
...
tk_new(Options, Interp) :-
	...
	illarg(domain(list(callable),tk_new_options), Goal, 1).
...
tk_option(top_level_events, App0, App, Disp0, Disp, Flags0, Flags) :-
	App=App0, Disp=Disp0,
	Flags is Flags0 \/ 0x0001.   % PROLOG_TK_NEW_FLAG_TOP_LEVEL_EVENTS
tk_option(disable_aqua_hack, App0, App, Disp0, Disp, Flags0, Flags) :- % [PM] 3.10.1
	App=App0, Disp=Disp0,
	Flags is Flags0 \/ 0x0002.   % PROLOG_TK_NEW_FLAG_DISABLE_AQUA_HACK
tk_option(enable_aqua_hack, App0, App, Disp0, Disp, Flags0, Flags) :- % [PM] 3.10.1
	App=App0, Disp=Disp0,
	Flags is Flags0 \/ 0x0004.  % PROLOG_TK_NEW_FLAG_ENABLE_AQUA_HACK
...
tk_do_one_event :-
	'$tk_do_one_event'(0x3f, 1).		% all + don't wait
...
tk_do_one_event1(Options) :-
	illarg(domain(list(atom),tk_do_one_event_options), tk_do_one_event(Options), 1).
...
is_option(tk_dont_wait,0x2).
is_option(tk_x_events,0x4).
is_option(tk_window_events,0x4).
is_option(tk_file_events,0x8).
is_option(tk_timer_events,0x10).
is_option(tk_idle_events,0x20).
is_option(tk_all_events,0x3d).
...
tk_next_event(Interp, Event) :-
	'$tk_do_one_event'(Interp, 0x3d, Event0),
	tk_num_main_windows(WindowsLeft),
	tk_next_event_cont(WindowsLeft, Interp, 0x3d, Event0, Event).

tk_next_event(Options, Interp, Event) :-
	integer(Options), !,
	M is Options /\ 0xfd,			% always wait
