...
static spio_t_error_code SPCDECL codesio_user_write(void *user_data, void *buf, size_t *pbuf_size, spio_t_bits write_options)
{
  spio_t_error_code code = SPIO_E_ERROR;
  codesio_t_open_codes *open_codes;
  size_t nitems;
                                        ...
                                        char const *str, SP_stream **streamp)
{
  ...
  if (str == NULL)              /* output stream */
  ...
  else                          /* input stream */
    {
      ...
      open_codes->index = 0;
      open_codes->len = 0;
      for (nchars = 0, p = str, q = open_codes->codes; (tmp = SP_wci_code(q, p)) > 0; p += tmp, q++, open_codes->len++)
        {
          ;                         /* empty */
        }
