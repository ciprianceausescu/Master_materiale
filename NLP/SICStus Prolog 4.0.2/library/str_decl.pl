...
new_compound_type(array(Type_expr), Type_name, Parent, N0, N,
		array(Elt_type), unknown, Alignment, Goal) --> !,
	get_type_name(Type_expr, Parent, N0, N, Elt_type, Elt_size0,
		Alignment, Goal),
	(   {integer(Elt_size0)} ->
		% we assume that an array should be aligned as it's elements
...
get_type_name(Name, _, N, N, Name, Size, Alignment, _) -->
	{atom(Name)},
...
struct_or_union_alignment([], Alignment, Alignment).
struct_or_union_alignment([Member|Rest], Alignment0, Alignment) :-
	...
	struct_or_union_alignment(Rest, Alignment2, Alignment).
...
term_exp(end_of_file, end_of_file) :-
	retractall(current_type_defn(_,_)),
	retractall(current_type_size(_,_,_)),
	retractall(current_type_file(_,_)),
	retractall(current_get_pred(_)),
	retractall(declared_multifile_discontiguous),
	retractall(declared_discontiguous_foreign).
