...
create_indexed_variables_body([A|B], [C|D], E, F, G, H, I) :-
        ...
        (   C\== +,
            ...
            (   L==empty ->
                hprolog_call(term_variables(A,E), H)
            ;   hprolog_call(term_variables(A,E,K), N),
                H=(N,L)
            ),
            I=M
        ;   E=K,
...
arggoal(A, B, C, D, E) :-
        (   A==[] ->
            ...
            C=true
        ;   A=[specinfo(_,any,_)] ->
            hprolog_call(term_variables(B,D,E), C)
        ;   C=(var(B)->D=[B|E];F),
...
generate_attr_unify_hook_one(A) :-
        ...
        hprolog_call(term_variables(D,O), P),
...
generate_attr_unify_hook_many(A, B) :-
        ...
        hprolog_call(term_variables(D1,L1), M1),
...
generate_indexed_variables_body(A/B, C, D, E) :-
        ...
        ;   create_indexed_variables_body(C, I, E, 1, A/B, K, L),
            (   K==empty ->
                ...
                E=[]
            ;   L==0 ->
                hprolog_call(term_variables(C,E), D)
            ;   K=D
            )
        ).
