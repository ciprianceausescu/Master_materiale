...
%% [PD] Quintus 3.5 We need something to emulate 'read_term_from_codes'
...
open_socket(Port, SocketOptions) :-
    open_socket(Port, prologbeans, SocketOptions).
...
set_socket_options(SocketOptions, Options) :-
    ...
    SocketOptions = [].
set_socket_options([reuseaddr], _Options).
...
data_received(Stream) :-
    ...
    catch( ( fast_read(IStream, Term), T=success ),
	  _E,
	  T=exception),
    ( T==success ->
