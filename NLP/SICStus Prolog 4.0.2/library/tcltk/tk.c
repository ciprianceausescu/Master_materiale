...
#define PROLOG_TK_NEW_FLAG_TOP_LEVEL_EVENTS  0x0001
#define PROLOG_TK_NEW_FLAG_DISABLE_AQUA_HACK 0x0002
#define PROLOG_TK_NEW_FLAG_ENABLE_AQUA_HACK  0x0004
...
/* Necessary ? */
  ...
  if (flags & PROLOG_TK_NEW_FLAG_TOP_LEVEL_EVENTS)
    {
...
#if (__APPLE__ && __MACH__)     /* 3.10.1 MacOS X */
  {
    ...
    {
      int disable_aqua_hack = ((flags & PROLOG_TK_NEW_FLAG_DISABLE_AQUA_HACK)
                               ||
                               getenv("SP_DISABLE_TCLTK_AQUA_HACK")/* another way to disable the hack */
                               );
      int enable_aqua_hack = ((flags & PROLOG_TK_NEW_FLAG_ENABLE_AQUA_HACK)
                              ||
...
long tk_do_one_event1(long flags)
{
  return Tk_DoOneEvent(flags);  /* a.k.a  Tcl_DoOneEvent */
}

#if SICSTUS_TODO
#error "[PM] 4.0 Do not hardwire Tcl/Tk flag values in tcltk.pl (duh!)"
#endif  /* SICSTUS_TODO */

void tk_do_one_event3(SP_term_ref tInterp, long flags, SP_term_ref tEvent)
{
...
static int SPCDECL tk_idle_hook(void *cookie)
{
  ...
  if (tk_NumMainWindows > 0
      && Tk_DoOneEvent(TCL_ALL_EVENTS|TCL_DONT_WAIT))
    {
#if DBG && 0
      fprintf(stderr, "tk_idle_hook called..");fflush(stderr);
