     ...
     int argno)
{
  ...
  switch (type&0xff)
    ...
    default:
      {
        char const *msg_ienc = msg_UTF8; /* [PM] 4.0 Tcl/Tk strings has string encoding close enough to SP ienc'd.  */
        SP_save_error(type, msg_ienc, culprit);
      }
    }
}

void sptcl_raise_error(char *name, int arity)
{
           ...
           )
{
  ...
  SP_ASSERT(write_options & SPIO_DEVICE_READ_OPTION_TEXT);
  SP_ASSERT(p->size >= p->index);
  (void)write_options;
...
/* Ignore compiler warnings here! */
      ...
      else if (name==local.atm_format && i==2)
	{
      ...
      else if (name==local.atm_codes && i==1)
	{
	  SP_get_arg(1, t1, t1);
