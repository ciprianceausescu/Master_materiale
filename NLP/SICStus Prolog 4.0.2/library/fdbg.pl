...
pretty_constraint(task_order(Tasks,Limit,Options), Pretty) :-
	cumulative_tasks(Tasks, Ss, Ds, Rs),
	(   Limit==1,
	    ones(Rs)
	->  Pretty = serialized(Ss, Options)
	;   Pretty = cumulative(Ss, Ds, Rs, Limit, Options)
	).

cumulative_tasks([], [], [], []).
cumulative_tasks([task(S,D,R)|Tasks], [S|Ss], [D|Ds], [R|Rs]) :-
	cumulative_tasks(Tasks, Ss, Ds, Rs).
...
protected_read_from_codes(Codes, Term) :-      %!! Is it OK to assume the
