...
%  NOTE:  library(str_decl) needs to be loaded at compile-time
...
foreign(pl_localtime, c, localtime(+clock, [-pointer(tm)])).
foreign(pl_gmtime, c, gmtime(+clock, [-pointer(tm)])).
foreign(pl_time, c, time(+clock, [-integer])).

foreign_resource(timestruct, [pl_localtime, pl_gmtime, pl_time]).

:- load_foreign_resource(timestruct).

%%% splfr --structs timestruct.pl timestruct.c
