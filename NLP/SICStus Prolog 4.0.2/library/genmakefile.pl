# -*- Perl -*-
# CVS: $Id: genmakefile.pl,v 1.7 2006/11/03 00:50:58 perm Exp $
#
...
print "\$(PLATFORM)/$resname.\$(FLI_SHSFX): $srcfile @d_objfiles\n";
print "\t\@echo\n";
print "\t\@echo \"Building \$@...\"\n";
print "\t\@echo =========================================\n";
print "\t\@echo\n";
# [PM] 3.12.3 Try to avoid issues when compiling on mips-irix-6.5:
# ld32: ERROR 48: Cannot access registry file ./so_locations (No locks available) - ignored.
# ld32: FATAL 51: Can't assign virtual addresses for bdb.so of size 10000 within specified range.  Please check your registry file ./so_locations.
print "\trm -f so_locations\n";
...
print "\t\$(MACOSX_MALLOC_SCRIBBLE_KLUDGE) \$(SPLFR) \$(SPLFRFLAGS) --header=$resname"."_tmp_header.h --resource=$resname \$($resname"."_d_splfrflags) \$(filter %.$objext %.pl,\$^) \$($resname"."_lib) \$(SPLFR_CFLAGS) \$($resname"."_d_splfr_copt) -o \$@\n";
...
print "\t\@echo\n";
# [PM] Do not specify --header here. It is built in a separate (earlier) pass by library/Makefile
print "\t\$(MACOSX_MALLOC_SCRIBBLE_KLUDGE) \$(SPLFR) \$(SPLFRFLAGS) --header=$resname"."_tmp_header.h --resource=$resname --static \$($resname"."_s_splfrflags) \$(filter %.$objext %.pl,\$^) \$($resname"."_lib) \$(SPLFR_CFLAGS) \$($resname"."_s_splfr_copt) -o \$@\n";
print "\trm -f $resname"."_tmp_header.h";
