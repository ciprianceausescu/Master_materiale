...
%@  to become defined by means of the @code{user:goal_expansion/3}
...
%@  @item @var{Module}:verify_attributes(@var{-Var}, @var{+Value}, @var{-Goals}) @hfill @dfn{hook}
...
%@  @var{Var} has been bound to @var{Value}.
%@  
...
%@  @code{call_residue/2}) and the Prolog top-level need to access
...
%@  @item @var{Module}:attribute_goal(@var{-Var}, @var{-Goal}) @hfill @dfn{hook}
...
%@  a goal is needed, for example in @code{frozen/2} and
%@  @code{call_residue/2}.  It should unify @var{Goal} with the
...
%@  @item @var{Module}:project_attributes(@var{+QueryVars}, @var{+AttrVars}) @hfill @dfn{hook}
%@  This predicate is called by the Prolog top level and by the
%@  built-in predicate @code{call_residue/2} in each module that
...
%@  @code{call_residue(@var{Goal},@var{Residue})}, when @var{Goal} has
%@  succeeded, @code{project_attributes/2} is called.  After that, all
%@  attributed variables created during the execution of @var{Goal}
%@  are located.  For each such variable, @code{attribute_goal/2}
%@  produces a term representing the constraint encoded by the
%@  attribute, and @var{Residue} is unified with the list of all
%@  such terms.
%@  
