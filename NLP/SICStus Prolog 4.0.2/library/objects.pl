...
create(Descriptor, Object) :-
	...
	;   var(Descriptor) ->
		(illarg(var,create(Descriptor,Object),
						    1))
	;   illarg(domain(term,create_spec),create(Descriptor,Object),1, Descriptor)
	).
...
allocate_instance(Descriptor, Object) :-
	...
	(   obj_decl:class_size(Class, Size, Header) ->
		...
		(   Address0 =:= 0 ->
		        illarg(resource(memory), create(Descriptor,Object))
		;   true
		),
...
destroy(Object) :-
	...
	;   illarg(domain(term,object),destroy(Object),1,Object)
	),
...
class_of(Object, Class) :-
	...
	;   class_of_hook(Object, Class1) ->
		Class = Class1
	;   illarg(type(object), class_of(Object, Class), 1, Object)
	).
...
descendant_of(Object, Class) :-
	...
	;   class_of_hook(Object, Class0) ->
		true
	;   illarg(type(object), descendant_of(Object, Class), 1, Object)
	),
...
pointer_object(Addr, Obj) :-
	(   integer(Addr) ->
		(   Addr =:= 0 ->
			Obj = null
		;   structs:get_atom(Addr, -4, Functor),
		    functor(Obj, Functor, 1),
		    arg(1, Obj, Addr)
		)
	;   nonvar(Addr) ->
		illarg(type(integer), pointer_object(Addr,Obj), 1, Addr)
	;   compound(Obj),
	    functor(Obj, _, 1) ->
		arg(1, Obj, Addr)
	;   Obj == null ->
		Addr = 0
	;   var(Obj) ->
		(illarg(var,pointer_object(Addr,Obj),
						    0))
	;   illarg(type(object), pointer_object(Addr,Obj), 2, Obj)
	).
...
ensure_can_send(Object, Op, Msg) :-
	...
	;   \+ callable(Msg) ->
		Goal =.. [Op,Object,Msg],
		illarg(domain(term,valid_message),Goal,2,Msg)
...
define_method(Object, Message, Body) :-
	...
	;   var(Body) ->
		(illarg(var,Goal,3))
	;   \+ compound(Object) ->
		illarg(type(object),Goal,3,Object)
	;   \+ callable(Message) ->
		illarg(type(valid_message),Goal,3,Message)
	;   \+ callable(Body) ->
		illarg(type(callable),Goal,3,Body)
	;   class_of(Object, Class),
	...
	;   illarg(domain(term,instance_method(Class)),Goal,3,Message)
	).
...
undefine_method(Object, Messagename, Arity) :-
	...
	;   \+ compound(Object) ->
		illarg(type(object),Goal,3,Object)
	;   \+ atom(Messagename) ->
		illarg(type(object),Goal,3,Messagename)
	;   \+ integer(Arity) ->
		illarg(type(object),Goal,3,Arity)
	;   class_of(Object, Class),
...
simple_expand_body(fetch_slot(Slot,Value), _, Class, _Object, Address, _Msg,
	...
	(   accessor(Class, Slot, Address, Value, Goal, _) ->
		true
	;   illarg(domain(term,valid_class_slot),fetch_slot(Slot,Value), 1,Slot)
	).
simple_expand_body(store_slot(Slot,Value), _, Class, _Object, Address, _Msg,
	...
	(   accessor(Class, Slot, Address, Value, _, Goal) ->
		true
	;   illarg(domain(term,valid_class_slot),store_slot(Slot,Value), 1, Slot)
	).
...
slot_name(Slot, Name) :-
	...
	;   Slot = [First|Rest],
		...
		concat_atom(Rest2, First, Name)
	    )
	).

concat_atom([], Atom, Atom).
concat_atom([Atom2|L], Atom1, Atom) :-
	atom_concat(Atom1, Atom2, Atom3),
	concat_atom(L, Atom3, Atom).
...
get_object(Address, Offset, Value) :-
	...
	(   Vaddr =:= 0 ->
		Value = null
	;   structs:get_atom(Vaddr,-4,Functor),
...
put_object_debug(Value, Address, Offset, Slot, Class) :-
	...
	;   illarg(domain(term,descendant_of(Class)),store_slot(Slot,Value), 2, Value)
	),
	arg(1,Value,Vaddr),
	structs:put_integer(Address, Offset, Vaddr).
