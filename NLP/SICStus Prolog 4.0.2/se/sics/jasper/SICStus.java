...
public class SICStus implements Prolog
{
    ...
    // [PM] Already in 3.8
// [PD] 3.11.1 See spInitialize<version> in library/jasper/spnative.c
//    private native int spInitialize(String argv[], String bootPath);
    ...
    // [PM] 3.8.5 Unused and bogus private native int spLoadLibfile(String qlFile);

    // private native long spQuery(SPPredicate pred, long predref, long arg_refs[]);
    private native long spCall(String module, String name, long arg_refs[]);

    // private native long spQueryCutFail(SPPredicate pred, long predref, long arg_refs[]);
    private native long spCallCutFail(String module, String name, long arg_refs[]);

    // private native long spOpenQuery(SPPredicate pred, long predref, long arg_refs[]);
    ...
    native long spOpenStringStream(String string); /* [PM] 3.9b5 *only* call from openStringStream */
    native boolean spCloseStringStream(long streamCode); /* [PM] 3.9b5 *only* call from closeStringStream */
    ...
    native Object spNewObject(Object clazz, String methname,
                              Object[] args, String typesig);
    native void spCallVoidMethodByName(Object obj, String methname,
                                       Object[] args, String typesig,
                                       boolean staticP);
    native Object spCallObjectMethodByName(Object obj, String methname,
                                           Object[] args, String types,
                                           boolean staticP);
    native boolean spCallBooleanMethodByName(Object obj, String methname,
                                             Object[] args, String types,
                                             boolean staticP);
    native byte spCallByteMethodByName(Object obj, String methname,
                                       Object[] args, String types,
                                       boolean staticP);
    native char spCallCharMethodByName(Object obj, String methname,
                                       Object[] args, String types,
                                       boolean staticP);
    native short spCallShortMethodByName(Object obj, String methname,
                                         Object[] args, String types,
                                         boolean staticP);
    native int spCallIntMethodByName(Object obj, String methname,
                                     Object[] args, String types,
                                     boolean staticP);
    native long spCallLongMethodByName(Object obj, String methname,
                                       Object[] args, String types,
                                       boolean staticP);
    native float spCallFloatMethodByName(Object obj, String methname,
                                         Object[] args, String types,
                                         boolean staticP);
    native double spCallDoubleMethodByName(Object obj, String methname,
                                           Object[] args, String types,
                                           boolean staticP);
    // [PD] 3.9 End of native methods used by class Jasper.
    ...
    /** This is the path where SICStus finds the Runtime Library. The
    ...
    boolean shouldCheckAgeFlag = true; // [PM] 3.9 default to true (was false in 3.8.6)
     ...
     * If the argument is <code>true</code> then the methods on {@link
     * se.sics.jasper.SPTerm SPTerm} will throw exceptions when used
     ...
     * <strong>Unsupported</strong>; see the release notes for details.
     * If the argument is <code>true</code> then {@link
     * se.sics.jasper.SPTerm#delete SPTerm.delete()} will make the
     * SP_term_ref associated with the deleted SPTerm object
     * available for re-use. Returns the old value of the flag.
     * <p>This flag is currently <strong>off</strong> by default.
     * Can be set with the
     * <code>se.sics.jasper.SICStus.reuseTermRefs</code> system property.
     */

    /** <strong>Unsupported</strong>; see the release notes for details.
     * If the argument is <code>false</code> then {@link
     * se.sics.jasper.SPTerm#delete SPTerm.delete()} will <strong>not</strong> make the
    ...
    /** Currently <strong>Unsupported</strong>. This method may
     ...
     * <code>se.sics.jasper.SICStus.debugLevel</code> system propery.  */
    ...
    /** [PM] 3.8.5 Top of the linked list of all live SPTerms.
     *  Each new SPTerm is pushed here. SPQuery close/cut/nextsolution/return to Prolog
     *  pops and invalidates from this stack to keep in synch with the
     *  deallocation done of term references on the Prolog side.
     *  Do not touch!
     **/
    SPTerm term_stack;
    /** [PM] 3.8.5 Top of the linked list of all open SPQuery-ies
     *  Each new SPQuery is pushed here. SPQuery close (and perhaps cut, nextsolution, close on older query)
     *  pops and invalidates from this stack to keep in synch with the
     *  Prolog side.
     *  Do not touch!
     **/
    ...
    void term_stack_unlink(SPTerm t, SPQuery start)
        throws SPException
    {
        SPTerm prev = null;
        SPTerm stack;
        ...
        for( prev = null, x = stack; x != t; x = x.next ) {
            if (x == null) {
                ...
                if (debugging()) SICStus.dbgPrintln(msg);
        ...
        // . start.term_stack_mark, start.next.term_stack_mark, ... a consecutive sequence
        // . sp.term_stack
        if (prev != null) {
            prev.next = t.next;
        }
        {
            SPQuery tmpQuery;
            if (debugging()) SICStus.dbgPrintln("Unlinking " + t.superToString() + "nativeTermRef=" + t.nativeTermRef);
            for (tmpQuery = start; tmpQuery != null && tmpQuery.term_stack_mark == t; tmpQuery = tmpQuery.next) {
                if (debugging()) SICStus.dbgPrintln("  From " + tmpQuery.superToString());
                tmpQuery.term_stack_mark=t.next;
            }
        }
        if (term_stack == t) {
            if (debugging()) SICStus.dbgPrintln("Unlinking from term_stack " + t.superToString());
            term_stack = t.next;
        }
    }
    ...
    /** This main-function is just a small test function. It will try
     * to load SICStus by creating a SICStus object and prints a
     * message if it succeeded.
    ...
    {
        if (api != 0) return SP_GLUEFAILURE;
        api = spapi;
        return SP_SUCCESS;
    }
    ...
    /* Load Jasper native code. To keep this simple, we let the OS
     * take care of searching by calling {@link
     * java.lang.System#loadLibrary() loadLibrary()}.
     * [PM] 3.8.4 Still true?
     * Usually, the jasper library (libjasper.dll or libjasper.so)
     * should be located in the same directory as the main SICStus
...
//    void loadNativeCode(String bootPath) {
    static void loadNativeCode(String bootPath) {
        if (debugLevelValueOrig>0) dbgPrintln("nativeCodeIsLoaded==" + nativeCodeIsLoaded);
        if (nativeCodeIsLoaded) return;
...
//                if (debugLevelValueOrig>0) dbgPrintln("unex==" + unex);
                ...
                try {
                    ...
                    if (debugLevelValueOrig>0) dbgPrintln("jarFileName==" + jarFileName);
                    ...
                    if (debugLevelValueOrig>0) dbgPrintln("libPath==" + libPath);
                    if (debugLevelValueOrig>0) dbgPrintln("realLibName==" + realLibName);
                    System.load(new File(libPath,realLibName).getPath());
                } catch (IOException ex) {
                    if (debugLevelValueOrig>0) System.err.println(ex);
                    throw new UnsatisfiedLinkError("Cannot locate " + realLibName);
                }
            }
        } else {                // Windows or !find_spnative_heuristic
//            if (debugging()) dbgPrintln("Calling System.loadLibrary(\"" + libName + "\")");
            if (debugLevelValueOrig>0) dbgPrintln("Calling System.loadLibrary(\"" + libName + "\")");
            System.loadLibrary(libName);
//            if (debugging()) dbgPrintln("Called System.loadLibrary(\"" + libName + "\")");
            if (debugLevelValueOrig>0) dbgPrintln("Called System.loadLibrary(\"" + libName + "\")");
        }
        ...
        if (!nativeVersion.equals(Version.SICStusVersion)) {
            throw new UnsatisfiedLinkError("Version of Class SICStus ("
                                           ...
                                           + ") does not match version of spnative ("
                                           + nativeVersion + ")");
        }
    ...
    void popToTermStackMark(SPTerm term_stack_mark)
    {
        synchronized (this) {
            ...
            while (term_stack != term_stack_mark) {
                ...
                // dbgPrintln("sp.term_stack.next==" + sp.term_stack.nativeTermRef);
    ...
    /**
     * Throws an exception if the current thread is not the thread which
     * created this instance of the SICStus object.
    ...
    {
          if (!isLegalCaller())
              throw new IllegalCallerException(this, attachedThread,Thread.currentThread());
    }
    
    /** Similar to {@link #checkLegalCaller() checkLegalCaller()}, but returns false
     * instead of throwing an exception.
     * @return true if the current thread is allowed to call methods in 
     * the SICStus-Prolog interface, false otherwise.
     */
    /* public (not public in 3.9) */
    ...
    // [PM] 3.8.5 Never did: throws IllegalCallerException
    {
        ...
        // return true;
        if (attachedThread == null) return true;
...
//      */
    ...
    static SICStus getNewSICStusFromAPIPtr(long api)
        throws SPException
    {
    ...
    synchronized private static void initSICStus(SICStus sp, String argv[], String bootPath)
        throws SPException
    {
        int rval;
...
//        dbgPrintln("entering initSICStus(sp, argv, bootPath)"); // *** [PD] TEMP
// [PM] multi-SP
//             if (SICStus.spGlobal != null) {
//                 throw new SPException(sp, "SICStus/JNI has already been initialized 1");
//             }
            // // DBG temporary
...
//            dbgPrintln("    getting debug level"); // *** [PD] TEMP
            debugLevelValueOrig = Integer.getInteger("se.sics.jasper.SICStus.debugLevel",
                                                     Integer.getInteger("se.sics.jasper.SICStus.debugLevelDefault",
                                                                        0)).intValue();
            ...
            sp.shouldCheckAgeFlag = 
                ( ( System.getProperty("se.sics.jasper.SICStus.checkSPTermAge") != null )
                  ?
                  // specified by user
                  Boolean.getBoolean("se.sics.jasper.SICStus.checkSPTermAge")
                  :
                  // default value
                  ( ( System.getProperty("se.sics.jasper.SICStus.checkSPTermAgeDefault") != null )
                    ?
                    Boolean.getBoolean("se.sics.jasper.SICStus.checkSPTermAgeDefault") // default specified
                    :
            ...
            sp.reuseTermRefsFlag = 
                ( ( System.getProperty("se.sics.jasper.SICStus.reuseTermRefs") != null )
                  ?
                  // specified by user
                  Boolean.getBoolean("se.sics.jasper.SICStus.reuseTermRefs")
                  :
                  // default value
                  ( ( System.getProperty("se.sics.jasper.SICStus.reuseTermRefsDefault") != null )
                    ?
                    Boolean.getBoolean("se.sics.jasper.SICStus.reuseTermRefsDefault") // default specified
                    :
                    // default is specified by initialization above
                    sp.reuseTermRefsFlag
                    )
            ...
            if (bootPath == null) {
                ...
                if (sp.debugging()) {
                    dbgPrintln("System.getProperty(\"sicstus.path\") == " + bootPath);
                }
                if (bootPath != null) {
                    bootPath += "/bin";
                }
            ...
            if (sp.debugging()) dbgPrintln("sp.bootPath = " + bootPath); // DBG
            ...
            // // multi-SP (but retain for backward compatibility. A ref to the *first* created sp)
            // if (SICStus.spGlobal == null) {
            //     SICStus.spGlobal = sp;
            // }
            ...
            if (sp.debugging()) {
                dbgPrintln("Maybe call sp.spInitialize031101(" + argv + ", " + sp.bootPath + "), api==" + sp.api); // DBG
            }

            if (sp.api == 0) {     // api != 0 if called from sicstus run-time
                if (sp.debugging()) dbgPrintln("Calling sp.spInitialize031101(" + argv + ", " + sp.bootPath + "), api==" + sp.api); // DBG
                rval = sp.spInitialize031101(argv, sp.bootPath);
                if (sp.debugging()) dbgPrintln("Called sp.spInitialize031101(" + argv + ", " + sp.bootPath + ") == " + rval + ", api==" + sp.api); // DBG
            ...
            // This is required so there is always a place to put the free list for SP_term_ref reuse
            sp.pushQueryMark();
    ...
    /** Creates a SICStus object. Equivalent to {@link #SICStus(String[],String) SICStus(null,null)}.
     *
     * @exception se.sics.jasper.SPException Something went wrong during startup.
    ...
    /** Creates a SICStus object. Equivalent to {@link
     * #SICStus(String[],String) SICStus(null,bootPath)}.
     * @param bootPath The path where SICStus should look for its
     * start-up files.
     * @exception se.sics.jasper.SPException Something went wrong during
     * startup.
     * @see se.sics.jasper.SPException 
     * @see se.sics.jasper.SICStus#SICStus(String[],String)
     */
    public SICStus(String bootPath)
        throws SPException
    {
        initSICStus(this, null,bootPath); 
    }
  
    /** Creates a SICStus object. This corresponds roughly to the
     * C-Prolog interface function <code>SP_initialize(argc, argv,
     * reserved)</code>. It initializes the emulator's memory manager,
     ...
     */
    public SICStus(String argv[], String bootPath)
        throws SPException
    {
//        dbgPrintln("in SICStus(argv, bootPath)"); // *** [PD] TEMP
     ...
     * Prolog code is through {@link se.sics.jasper.SICStus#restore
     * restore}.
     *
     * @param file The Prolog code to load. This can be a .pl-file, a
     * .ql-file, or a .po-file.
     * @exception se.sics.jasper.SPException A Prolog exception was thrown.
     * @see se.sics.jasper.SICStus#restore */
    synchronized public void load(String file)
        throws SPException // , IllegalCallerException
    {
    ...
    /**
     * Interface to <code>SP_restore()</code>. Restores the specified
     * .sav-file.  This is the preferred way of loading Prolog code
     * instead of using {@link se.sics.jasper.SICStus#load load}.
     ...
     * </pre>
     *
     * @param savFile The .sav-file to restore.
     * @see se.sics.jasper.SICStus#load */
    synchronized public void restore(String savFile)
        throws SPException // , IllegalCallerException
    {
        // [PM] 3.8.5 checkLegalCaller();
        checkLegalCaller();     // 3.9
...
/* [PD] 3.9 library(jasper) is no longer loaded, so don't reload it.
        ...
        //          unloads foreign resources (via SP_restore()) .
        //          This should perhaps be done in a different way, e.g. by
        //          making SP_restore() not unload library(jasper).
...
*/
     ...
     * <code>SP_deinitialize</code> to deallocate and unload resources
    ...
    */
    ...
    long[] termArrayToLongArray(SPTerm args[])
        throws IllegalTermException
    {
        ...
        {
            args[i].CheckValid(this); // [PM] 3.8.5 Perhaps wrap in getNativeTermRef()
    ...
    long makeTermRef()
    {
        ...
        if (termRef == SPTerm.SP_ILLEGAL_TERM_REF) {
            ...
            // Detect if any problems (not expected but [PM] do not want this feature to blow up in his face.)
            if (!spValidTermRef(termRef)) {
                // turn the broken feature off. (not fail proof, may be inside readfromstring that binds it to true.)
                setReuseTermRefs(false);
                termRef = spMakeTermRef(); // not needed, given the exception thrown below
...
//                     // internal error
//                     throw new SPException(this, "ERROR: Internal error in spgetExceptionTerm");
//                 }
...
//     }
    ...
    private int exceptionTermPrintDepth =
        Integer.getInteger("se.sics.jasper.SICStus.printDepth", 5).intValue();
     ...
     * {@link se.sics.jasper.SPException#toString() toString} method
     * of the class {@link se.sics.jasper.SPException SPException}
     * returns).
     *
     * @param level The new maximum print depth for exception terms.
     * @return The old maximum print depth value. 
    ...
    boolean handleQueryResult(long result)
        throws SPException
    {
        // DBG
        if (debugging(2)) dbgPrintln("handleQueryResult() called with result = " + result);
        // Thread.dumpStack();
        // [PM] 3.8.5 Used to coerce to int and then switch.
        if (result == SP_SUCCESS) return true;
        if (result == SP_FAILURE) return false;
        if (result == SP_ERROR)
            {
                //SPTerm spe;
                int speIndex;
                ...
                // DBG
                if (debugging(2)) dbgPrintln("SP_ERROR in handleQueryResult().\n");
                ...
                // DBG
      // [PD] 3.10.2 Use higher debug level here. spe might be a circular term.
                //if (debugging(3)) dbgPrintln("SP_ERROR: `" + spe + "'\n");
      // [PD] 3.10.2 Back to level 2. We no longer a term to print.
                if (debugging(2)) dbgPrintln("SP_ERROR: `" + speIndex + "'\n");
                ...
                if (speIndex != 0) {
                    ...
                    long streamCode = openStringBufferStream(sb);
                    String termString =
                        ( spExceptionString(speIndex, streamCode,
                                            exceptionTermPrintDepth)
                          ? sb.toString() : null );
                    closeStringStream(streamCode);
                    e = new SPException(this, termString, speIndex);
                    // DBG
                    //if (debugging()) dbgPrintln("handleQueryResult() created e (spe!=null).\n");
                    if (debugging()) dbgPrintln("handleQueryResult() created e (speIndex!=0).\n");
                    throw e;
                } else {
                    // [PD] 3.10.2
                    //e = new SPException(this, spe);
    ...
    SPTerm newGlueTerm()
    {
        ...
        if (debugging()) SICStus.dbgPrintln("Calling "+this+".newGlueTerm()");
        return new SPTerm(this, (long)0 /* used as special flag, note the importance of 'long' */);
    }
    ...
    {
        // [PD] 3.9.2 Check if debugging before calling debug code ...
        if (debugging()) SICStus.dbgPrintln("Calling newGlueTerm(" + nativeTermRef + ")");
        return new SPTerm(this, nativeTermRef);
    }

    // [PM] 3.9 called by glue
    SPCanonicalAtom newGlueAtom(String name)
        throws AtomRegisterFailure // cannot happen (never thrown)
    {
        // [PD] 3.9.2 Check if debugging before calling debug code ...
        if (debugging()) SICStus.dbgPrintln("Calling newGlueAtom(\"" + name + "\")");
     ...
     *
     * @param module The module used for the query.
     * @param name The predicate name used for the query.
     * @param args The arguments to the predicate. The number of
     * arguments must match the functor of the predicate.
     * @return true/false corresponding to success/failure of the query.
     * @exception se.sics.jasper.SPException If a Prolog exception was thrown.
    ...
    {
        checkLegalCaller();     // 3.9
        synchronized (this) {   // This restriction is really unfortunate but (almost) unavoidable
            return
                handleQueryResult(spCall(module, name, termArrayToLongArray(args)));
        }
    }

    /** Finds the first solution to a query specified as a string. If
     * you need more than one solution, use {@link
     * se.sics.jasper.SICStus#openQuery openQuery}.
     * 
    ...
    /** <strong>Deprecated</strong> Use module and predicate name directly instead.
     * Finds the first solution to a query. If you need more than one
     * solution, use {@link se.sics.jasper.SICStus#openQuery openQuery}. 
     *
     * @param pred The predicate object to use for the query.
     * @param args The arguments to the predicate. The number of
     * arguments must match the functor of the predicate.
     * @return True/false corresponding to success/failure of the query.
     * @exception se.sics.jasper.SPException If a Prolog exception was thrown.
    ...
    public boolean query(SPPredicate pred, SPTerm args[])
        throws SPException, IllegalTermException // , IllegalCallerException
    {
        checkLegalCaller();     // 3.9
     ...
     * side-effects during the first solution.
     *
     * @param module The module used for the query.
     * @param name The predicate name used for the query.
        ...
        throws SPException
    {
        ...
        synchronized (this) {   // This restriction is really unfortunate but (almost) unavoidable
            return 
                handleQueryResult(spCallCutFail(module, name, termArrayToLongArray(args)));
        }
    }
     ...
     * side-effects during the first solution.
     *
     * @param pred The predicate object used for the query.
     * @param args The arguments to the predicate. The number of
     * arguments must match the functor of the predicate.
     * @return True/false corresponding to success/failure of the query.
     * @exception se.sics.jasper.SPException
     ...
     * does not find any solutions; use the method {@link
     * se.sics.jasper.SPQuery#nextSolution nextSolution} to do
     * that. When no more solutions are needed, the query must be closed
     * using methods {@link se.sics.jasper.SPQuery#close close} or
     * {@link se.sics.jasper.SPQuery#cut cut} on the  query-object.
     * <p> Multiple queries can be open at the same
     * time. See {@link se.sics.jasper.SPQuery#nextSolution
     * nextSolution} for details and restrictions.
     *
     * @param module The module used for the query.
     * @param name The predicate name used for the query.
     * @param args The arguments to the predicate. The number of
     ...
     */
    public SPQuery openQuery(String module, String name, SPTerm args[] ) // NOTE: move most of this to SPQuery
     ...
     * the method {@link se.sics.jasper.SPQuery#nextSolution
     * nextSolution} to do that. When no more solutions are needed,
     * the query must be closed using methods {@link
     * se.sics.jasper.SPQuery#close close} or {@link
     * se.sics.jasper.SPQuery#cut cut} on the query-object.

     * <p>

     * Multiple queries can be open at the same time. See {@link
     * se.sics.jasper.SPQuery#nextSolution nextSolution} for details
     * and restrictions. 

     * <p>

     * Allocates one {@link
     * se.sics.jasper.SPTerm SPTerm} for each entry added to the map
     ...
     * nextSolution} for details and restrictions.
     *
     * @param pred The predicate-object to open a query on.
     * @param args The arguments to the predicate. The number of
     * arguments must match the functor of the predicate.
     *
     * @return The openened {@link se.sics.jasper.SPQuery query} object.
     ...
     * @see se.sics.jasper.SPPredicate 
     */
    public SPQuery openQuery(SPPredicate pred, SPTerm args[] ) // NOTE: move most of this to SPQuery
        throws IllegalTermException, SPException
    {
        checkLegalCaller();     // 3.9
    ...
    private void checkCallArity(SPPredicate pred, int arity)
        throws SPException
    {
        if (pred.arity != arity) {
            throw new SPException(this,
                                  pred.module + ":" + pred.name + "/" + pred.arity
                                  + " called with "
                                  + arity
                                  + "arguments");
        }
    ...
    /* not yet public */ SPQuery openContext()
        throws SPException
    {
        checkLegalCaller();     // 3.9

        // SPPredicate true_predref = new SPPredicate(this, "true", 0, "prolog");
    ...
    // Push a faked query on the query stack. Used to keep track of positions in the SPTerm stack.
        ...
        // synchronized (this) {
            ...
            if (debugging()) {
                ...
                if (debugging(2)) {
                    ...
                    if (term_stack != null) term_stack.dumpTermStack();
                }
            }
    ...
    void pushSPGlobal()
    {
        ...
        if (debugging(2)) {
            SICStus.dbgPrintln("Enter pushSPGlobal()"
                               + ", this==" + this
                               + ", stack==" + stack
    ...
    static SICStus topSPGlobal()
    {
        ...
        if (debugLevelValueOrig>0) { // [PM] ugly but have no static debugging() method
            SICStus.dbgPrintln("Enter topSPGlobal()"
                               + ", stack==" + stack
                               + ", thread==" + Thread.currentThread()
                               );
        }
    ...
    void popSPGlobal()
    {
        ...
        if (debugging(2)) {
            SICStus.dbgPrintln("Enter popSPGlobal()"
                               + ", this==" + this
                               + ", stack==" + stack
                               + ", thread==" + Thread.currentThread()
                               );
        ...
        if (stack == null) {
            if (debugging()) SICStus.dbgPrintln("ERROR: spGlobalStack==null this==" + this + ", thread==" + Thread.currentThread());
        }
        Object tmp = stack.pop();
        // defaultSPGlobal should never be re-set.

        if (tmp != this) {
            if (debugging()) SICStus.dbgPrintln("ERROR: spGlobalStack.pop() != this (" + tmp + "!=" + this + "), thread==" + Thread.currentThread());
        }
    }
    ...
    /* Was used briefly when reuseTermRefs was off by default
    SPTerm newTerm()
    {
        synchronized (this) {
            SPTerm t;
            boolean old = reuseTermRefs();
            try {
                setReuseTermRefsFlag(true);
                t = new SPTerm(this);
            } finally {
                setReuseTermRefsFlag(old);
            }
            return t;
        }
    }
    */
    ...
    void freeTerm(java.util.Set freeList, SPTerm term)
        throws SPException
    {
        if (freeList != null) {
        ...
        throws SPException
    {
        ...
        if (!varMap.isEmpty()) {
            ...
            while(itr.hasNext()) {
                ...
                if (debugging()) SICStus.dbgPrintln("toKeyList: entry.getKey()==" + entry.getKey());
    ...
    void mergeKeyList(java.util.Map varMap, SPTerm keyList, java.util.Set freeList)
        throws SPException
    {
        ...
        while(tail.isList()) {
            ...
            // If not in varMap then only add if the name does not start with underscore
    ...
    {
        /* [PM] 3.9b5 the string stream handling has a global list
         * shared by all SICStus run-times. We serialize access to
         * this list by always locking the whole class */
        ...
        throws SPException
    {
        ...
        synchronized (this) {
            ...
            try {
                ...
                term = newTerm(freeList).putVariable(); // -Term (outside context)
            
                mapIsReadOnly = (varMap == null || false /* need a way to detect un-put-able maps*/);
                ...
                try {
                    ...
                    // jasper_read_from_string(+StreamCode, +VarsIn, -VarsOut, -Term)
     ...
     */
    public SPTerm readFromString(String string)
        throws SPException
    {
        checkLegalCaller();     // 3.9
     ...
     * 
     * If you need more than one solution, use {@link
     * se.sics.jasper.SICStus#openQuery
     * openQueryFromString}.
     * 
     * @param string The goal to use for the query, with terminating period.
     * @param varMap The arguments to the predicate as a map from
        ...
        throws SPException
    {
        ...
        synchronized (this) {
            ...
            jstreamCode = openStringStream(string);
            if (debugging(2)) SICStus.dbgPrintln("queryFromString: jstreamCode==" + jstreamCode);
            try {
                java.util.Set freeList = newFreeList();
                SPTerm streamCode;
                SPTerm varsIn;
                ...
                if (mapIsReadOnly) {
                    context = openContext();
                    if (debugging()) SICStus.dbgPrintln("creating context age()=" + context.age());
                }
                try {
                    streamCode = newTerm(freeList).putInteger(jstreamCode);
                    varsIn = ((varMap != null) ? toKeyList(varMap, freeList) : newTerm(freeList));
                    ...
                    if (result && !mapIsReadOnly) { // add new variables to varMap on success
                        mergeKeyList(varMap, varsOut, freeList);
                    }
                    freeTerm(freeList, varsOut);
                }
    ...
    // version.
    // /** <strong>Experimental:</strong> Equivalent to queryFromString(string, null);
    //  */
    ...
    /** 
     * This is to {@link se.sics.jasper.SICStus#queryCutFail queryCutFail}
     * what {@link se.sics.jasper.SICStus#queryFromString queryFromString}
     * is to {@link se.sics.jasper.SICStus#query query}.
     *
     * @param string The goal to use for the query, with terminating period.
     * @param varMap The arguments to the predicate as a map from
     * variable names to SPTerm objects. May be null. No bindings are
    ...
    /**
     * This is to {@link se.sics.jasper.SICStus#openQuery openQuery}
     * what {@link se.sics.jasper.SICStus#queryFromString queryFromString}
     * is to {@link se.sics.jasper.SICStus#query query}.
     *
     * Allocates one {@link se.sics.jasper.SPTerm SPTerm} for each
     ...
     * objects for internal use). If varMap is null then this method
     * behaves as if no SPTerm objects were created.
     * When the query is closed (or cut) then all SPTerm objects are
     * reclaimed.
     * 
     * @param string The goal to use for the query, with terminating period.
     * @param varMap The arguments to the predicate as a map from
        ...
        throws SPException
    {
        ...
        SPQuery query;
        SPTerm goal;
        synchronized (this) {   // protect time span between openContext and setContext
            context = openContext();
            try {
                // DBG
                if (debugging(2)) dbgPrintln("openQueryFromString readFromString(" + string + "), varMap==" + varMap);
            
                goal = readFromString(string, varMap);
                // DBG
                if (debugging(2)) dbgPrintln("openQueryFromString goal==" + goal + ", varMap==" + varMap);
                ...
                // DBG
                if (debugging(2)) dbgPrintln("openQueryFromString finally context==" + context);
                if (context != null) {
                    context.close();
                }
            }
    ...
    public Term newObjectTerm(Object obj)
    ...
    * create a server and a client ({@link Prolog}) interface for this SICStus
    ...
    * {@link se.sics.jasper.SICStus#stopServer}.
