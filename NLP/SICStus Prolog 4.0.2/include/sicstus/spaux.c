...
/* [PM] 4.0 non-negative exception type if a $reserved/1 exception was
   ...
   return -1 if no proerly formatted $reserved/1 exception was present
*/
static int get_reserved_exception(long *pdata)
{
  long res = -1;
...
int SPCDECL SPLD_MAIN_NAME(int argc, char *argv[])
{
  ...
  int inited=0;                 /* glue_initialize successfully called? */
  int result = 0;
  ...
  sp_argc = argc-1;
  sp_argv = &argv[1];
  rv = sp_glue_initialize(sp_argc,sp_argv,NULL,sp_pre_linkage,sp_pre_map,dsp,SP_GLUE_INITIALIZE_OPTION_RESTORE);
 ...
 cleanup:
  if (inited) (void)SP_deinitialize();
  return result;
}
...
/* foreign resource include this file just before 
   ...
   make it static   */
#if 1                           /* [PM] 4.0 I have removed the extern declaration from spaux.h.in, see if static is OK always. */
static
#else  /* 0 */
#if INCLUDED_IN_FOREIGN_RESOURCE
...
#if SP_SINGLE_THREADED
#error "[PM] 4.0 FIXME: Need to investigate interaction between SP_MANGLE, SP_MAINFUN_PARAM_FLAG_MULTI etc"
#endif  /* SP_SINGLE_THREADED */

  /* Check if more than one SICStus is trying to use this resource */
#if (MULTI_SP_AWARE || SP_SINGLE_THREADED)
  /* nothing, multiple use is OK. */

  params->flags |= SP_MAINFUN_PARAM_FLAG_MULTI;

#else  /* !(MULTI_SP_AWARE || SP_SINGLE_THREADED) */

  params->flags |= SP_MAINFUN_PARAM_FLAG_PLAIN;

#if SP_DYNAMIC_FOREIGN_RESOURCE
  *papi = api; /* must be done even if !SP_MAINFUN_PARAM_FLAG_ENTER so that caller can use SP_XXX on unload too.*/

  {
  /* sharing between multiple sicstus can only happen for shared
     library versions of a foreign resource. */
  if (params->flags & SP_MAINFUN_PARAM_FLAG_ENTER)
    {
...
#endif /* SP_DYNAMIC_FOREIGN_RESOURCE */
#endif /* !(MULTI_SP_AWARE || SP_SINGLE_THREADED) */
