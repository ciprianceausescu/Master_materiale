...
#define SP_DISPATCH_API_VERSION ((int)((SP_DISPATCH_API_VERSION_MAJOR<<16)+SP_DISPATCH_API_VERSION_MINOR)) /* 0x201 */
...
typedef int (SPCDECL tsp_glue_initialize) PROTOTYPE((int, char * *, void *, SP_MainFun * *, char * *, int, int));
...
typedef SP_pred_ref (SPCDECL tSP_predicate) PROTOTYPE((char const *, long, char const *));
typedef SP_pred_ref (SPCDECL tSP_pred) PROTOTYPE((SP_atom, long, SP_atom));
...
typedef int (SPCDECL tSP_event) PROTOTYPE((SP_EventFun *, ANYPOINTER));
...
typedef int (SPCDECL tSP_put_integer) PROTOTYPE((SP_term_ref, long));
...
typedef int (SPCDECL tSP_put_address) PROTOTYPE((SP_term_ref, ANYPOINTER));
...
typedef int (SPCDECL tSP_get_integer) PROTOTYPE((SP_term_ref, long *));
...
typedef int (SPCDECL tSP_get_address) PROTOTYPE((SP_term_ref, ANYPOINTER *));
...
typedef spio_t_error_code (SPCDECL tSP_printf) PROTOTYPE((char const * fmt, ...));
typedef spio_t_error_code (SPCDECL tSP_fprintf) PROTOTYPE((SP_stream * stream, char const * fmt, ...));
typedef spio_t_error_code (SPCDECL tsp_ttyprintf) PROTOTYPE((char const * fmt, ...));
...
typedef SP_SigFun * (SPCDECL tSP_signal) PROTOTYPE((int, SP_SigFun *, ANYPOINTER));
...
typedef ANYPOINTER (SPCDECL tSP_malloc) PROTOTYPE((size_t));
typedef ANYPOINTER (SPCDECL tSP_realloc) PROTOTYPE((ANYPOINTER, size_t));
typedef void (SPCDECL tSP_free) PROTOTYPE((ANYPOINTER));
typedef void (SPCDECL tSP_save_error) PROTOTYPE((int error_type, char const * error_msg_ienc, SP_term_ref culprit));
typedef void (SPCDECL tSP_raise_error) PROTOTYPE((char const * name, int arity, int argn));
#if !SP_WIN32
...
#endif /* SP_WIN32 */
typedef int (SPCDECL tSP_atom_length) PROTOTYPE((SP_atom));
...
typedef void (SPCDECL tsp_set_abort_env) PROTOTYPE((ANYPOINTER));
...
typedef int (SPCDECL tSP_set_initial_options) PROTOTYPE((struct SP_options *));
...
typedef ANYPOINTER (SPCDECL tsp_get_engine_global) PROTOTYPE((void));
typedef void (SPCDECL tsp_reset_ctrlc_pending) PROTOTYPE((void));
typedef int (SPCDECL tsp_get_ctrlc_pending) PROTOTYPE((void));
...
typedef int (SPCDECL tsp_get_mem_usage) PROTOTYPE((void));
...
typedef int (SPCDECL tSP_external_object_default_putter_helper) PROTOTYPE((SP_term_ref tr, SP_external_object_link * obj, unsigned long functor));
...
typedef void (SPCDECL tSP_finalize_external_object) PROTOTYPE((SP_term_ref tObj, long * existed));
typedef long (SPCDECL tSP_garbage_collect_external_objects) PROTOTYPE((SP_external_object_type object_type));
...
typedef int (SPCDECL tsp_spio_trace_line) PROTOTYPE((char const * function, char const * file, int line, char const * msg, long l, char const * string, int level));
...
typedef int (SPCDECL tSP_decode_reserved_exception) PROTOTYPE((SP_term_ref tr, long * pdata));
...
typedef struct DISPATCH_TABLE_STRUCT_SICSTUS_H {
   unsigned long size; /* sizeof DISPATCH_TABLE_STRUCT_SICSTUS_H */
   const char *name;   /* "sicstus.h" */
   int version;        /* 0x201  Major: 2, Minor: 1 */
   ...
   const char *nSP_save_error; tSP_save_error *pSP_save_error; /* API #105 */
   const char *nSP_raise_error; tSP_raise_error *pSP_raise_error; /* API #106 */
#if !SP_WIN32
   ...
   const char *nsp_reset_ctrlc_pending; tsp_reset_ctrlc_pending *psp_reset_ctrlc_pending; /* API #127 */
   const char *nsp_get_ctrlc_pending; tsp_get_ctrlc_pending *psp_get_ctrlc_pending; /* API #128 */
   ...
   const char *nsp_get_mem_usage; tsp_get_mem_usage *psp_get_mem_usage; /* API #131 */
...
#define SP_API_SICSTUS_H_NUMBER_OF_ENTRIES 189
#if INCLUDED_FROM_RUNTIME /* No indirection within SP runtime */
...
extern tSP_get_list_n_bytes SP_get_list_n_bytes;
   ...
   #define SP_save_error (GetSICStusAPIProc(API_SICSTUS_H,pSP_save_error,nSP_save_error,"SP_save_error"))
   #define SP_raise_error (GetSICStusAPIProc(API_SICSTUS_H,pSP_raise_error,nSP_raise_error,"SP_raise_error"))
#if !SP_WIN32
...
#endif /* SICSTUS_HIDDEN_API */
#if SICSTUS_HIDDEN_API || SICSTUS_UNHIDE_sp_reset_ctrlc_pending
   #define sp_reset_ctrlc_pending (GetSICStusAPIProc(API_SICSTUS_H,psp_reset_ctrlc_pending,nsp_reset_ctrlc_pending,"sp_reset_ctrlc_pending"))
#endif /* SICSTUS_HIDDEN_API */
#if SICSTUS_HIDDEN_API || SICSTUS_UNHIDE_sp_get_ctrlc_pending
   #define sp_get_ctrlc_pending (GetSICStusAPIProc(API_SICSTUS_H,psp_get_ctrlc_pending,nsp_get_ctrlc_pending,"sp_get_ctrlc_pending"))
#endif /* SICSTUS_HIDDEN_API */
...
#endif /* SICSTUS_HIDDEN_API */
#if SICSTUS_HIDDEN_API || SICSTUS_UNHIDE_sp_get_mem_usage
   #define sp_get_mem_usage (GetSICStusAPIProc(API_SICSTUS_H,psp_get_mem_usage,nsp_get_mem_usage,"sp_get_mem_usage"))
#endif /* SICSTUS_HIDDEN_API */
...
typedef void (SPCDECL tcall_overflow) PROTOTYPE((Argtype Arg, int pad, int arity));
...
typedef TAGGED (SPCDECL tsp_make_integer) PROTOTYPE((Argtype Arg, SP_INTEGER i));
typedef SP_BOOL (SPCDECL tcunify) PROTOTYPE((Argtype Arg, TAGGED u, TAGGED v));
typedef void (SPCDECL tinit_ds) PROTOTYPE((struct init_ds_arg *));
typedef struct sw_on_key_node * (SPCDECL tincore_gethash) PROTOTYPE((struct sw_on_key * sw, TAGGED k));
typedef void (SPCDECL tchoice_overflow) PROTOTYPE((Argdecl, int pad));
...
typedef struct sw_on_key * (SPCDECL tnew_switch_on_key) PROTOTYPE((int size, struct try_node * otherwise));
...
typedef int (SPCDECL tSP_install_fd_hooks) PROTOTYPE((SP_HOOKS_COOKIE, SP_FD_ManagerFun *, SP_FD_RestoreFun *, SP_FD_SaveFun *, SP_FD_DestructorFun *, int));
typedef void (SPCDECL treinitialize) PROTOTYPE((Argtype));
typedef void (SPCDECL tintoRelocationChain) PROTOTYPE((Argtype Arg, TAGGED * chain, TAGGED * elem));
typedef void (SPCDECL tupdate_exports) PROTOTYPE((struct definition * f, int type));
typedef int (SPCDECL tlarge_insn_offset) PROTOTYPE((INSN *, INSN *));
typedef void (SPCDECL tsp_exitmsg) PROTOTYPE((int));
typedef char * (SPCDECL tsp_expand_file_name) PROTOTYPE((char const * relpath, char const * cwd, spio_t_bits options));
typedef int (SPCDECL terrprintf) PROTOTYPE((int flags, const char * fmt, ...));
typedef void (SPCDECL tdelete_attributes) PROTOTYPE((TAGGED, struct mod_def *));
...
typedef void (SPCDECL tset_enter_instr) PROTOTYPE((struct definition *, int));
...
typedef struct DISPATCH_TABLE_STRUCT_SUPPORT_H {
   unsigned long size; /* sizeof DISPATCH_TABLE_STRUCT_SUPPORT_H */
   ...
   const char *ninit_ds; tinit_ds *pinit_ds; /* API #8 */
   const char *nincore_gethash; tincore_gethash *pincore_gethash; /* API #9 */
   const char *nchoice_overflow; tchoice_overflow *pchoice_overflow; /* API #10 */
   ...
   const char *nreinitialize; treinitialize *preinitialize; /* API #20 */
   ...
   const char *nsp_expand_file_name; tsp_expand_file_name *psp_expand_file_name; /* API #25 */
   ...
   const char *nset_enter_instr; tset_enter_instr *pset_enter_instr; /* API #30 */
   const char *nmarkTerm; tmarkTerm *pmarkTerm; /* API #31 */
   const char *nunavailable_api_32; void *unavailable_api_32; /* API #32 */
#if SP_WIN32
...
#define SP_API_SUPPORT_H_NUMBER_OF_ENTRIES 35
#if INCLUDED_FROM_RUNTIME /* No indirection within SP runtime */
...
extern tSP_set_errprintf_hook SP_set_errprintf_hook;
   ...
   #define init_ds (GetSICStusAPIProc(API_SUPPORT_H,pinit_ds,ninit_ds,"init_ds"))
   #define incore_gethash (GetSICStusAPIProc(API_SUPPORT_H,pincore_gethash,nincore_gethash,"incore_gethash"))
   #define choice_overflow (GetSICStusAPIProc(API_SUPPORT_H,pchoice_overflow,nchoice_overflow,"choice_overflow"))
   ...
   #define reinitialize (GetSICStusAPIProc(API_SUPPORT_H,preinitialize,nreinitialize,"reinitialize"))
   ...
   #define sp_expand_file_name (GetSICStusAPIProc(API_SUPPORT_H,psp_expand_file_name,nsp_expand_file_name,"sp_expand_file_name"))
   ...
   #define set_enter_instr (GetSICStusAPIProc(API_SUPPORT_H,pset_enter_instr,nset_enter_instr,"set_enter_instr"))
...
#if DBG
/* [PM] 3.11.2 whether the debug fields 'dummy', 'name' and 'mainfunc' should be available */
...
#define SetupSICStusDISPATCH() 0
#else  /* !MULTI_SP_AWARE */
#ifndef SetupSICStusDISPATCH
#define SetupSICStusDISPATCH() (((*GetSICStusDISPATCHAddress()) == 0) ? (*(GetSICStusDISPATCHAddress())) = SP_get_dispatch(NULL) : 0) 
#endif /* SetupSICStusDISPATCH */
#endif /* !MULTI_SP_AWARE */

#ifndef GetSICStusAPIProc
#define GetSICStusAPIProc(API, PTRFIELD_NAME, NAMEFIELD_NAME/*IGNORED*/, NAME_AS_STRING/*IGNORED*/) (*((GetSICStusDISPATCH(API))->PTRFIELD_NAME))
...
#if (MULTI_SP_AWARE && (SP_DYNAMIC_FOREIGN_RESOURCE || SPDLL))
/* A dynamic multi-sp resource passes around the API, no global variable used.
...
#if 0                           /* [PM] 4.0 is global declaration needed? */
extern int SPCDECL sp_main_helper(
