...
#define SPIO_PRIVATE_OPTION_0_  (1 << (SPIO_OPTION_PRIVATE_BIT_OFFSET+ 0)) /* only use as SPIO_OPTION_USE_PRIVATE */
...
#define SPIO_PUSH_OPTION_SEEKABLE_BACK SPIO_NEXT_BIT(SPIO_PUSH_OPTION_POPPABLE) /* Try to ensure layer can be back-seeked (expensive in general) */

#if 0
   #define SPIO_PUSH_OPTION_READ_EOL_AUTO SPIO_NEXT_BIT(SPIO_PUSH_OPTION_SEEKABLE_BACK)
...
/* seekability (do we really need these? Consider making them private in spio_layer_unix.c) YES: SPIO_LAYER_FLAG_SEEKABLE_DATA (or make it mandatory) */
...
#define SPIO_READ_GRANULARITY(FLAGS) (                          \
...
#define SPIO_WRITE_GRANULARITY(FLAGS) (                          \
...
/* interrupting */
extern void spio_interrupt(void); /* schedules all interrupt hooks to run (async-signal safe) */
...
/* FIXME: add proper thread-safety locks */
