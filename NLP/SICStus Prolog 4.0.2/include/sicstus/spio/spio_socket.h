...
/*
  SPIO_S_TRUE (SPIO_S_FALSE) if a and b have the same (different) address (e.g. for comparing getpeername and getsockname addresses).
  Error code for unknown protocol or other error.
*/
extern spio_t_error_code spio_socket_addr_equal(struct sockaddr const *a, struct sockaddr const *b);
