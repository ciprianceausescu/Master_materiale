...
def rotateImage(imageName):
    ...
    if y1 < y4:
        while y1 not in range(y4-1,y4+1):
            image = cv2.imread(imageName, 1)
            rowCount, colCount, _ = image.shape
            M = cv2.getRotationMatrix2D((colCount/2,rowCount/2),0.03,1) #0.1 / 0.2 pt testele scanate
            dst = cv2.warpAffine(image,M,(colCount,rowCount),flags=cv2.INTER_CUBIC)
    ...
    if y1 > y4:
        while y1 not in range(y4-1,y4+1):
            image = cv2.imread(imageName, 1)
            rowCount, colCount, _ = image.shape
            M = cv2.getRotationMatrix2D((colCount/2,rowCount/2),-0.03,1) #-0.03
